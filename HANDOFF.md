...
**SAVE-UX-WARN-AGGREGATION 設計**: `docs/specs/20260513_shogi_save_ux_warn_aggregation_design.md`（warn 集約・kind 分類・三層分離原則の設計。v1 追補 §15 に PR #70 〜 #76 の到達点を反映: A-5.1 SAVE 系 15/15 helper 経由化完了 / metadata 土台 / `save-verify:{core,entry,edit,past,pairing}` の Group 対応 / `showMsg` 3000ms 集約 / 短縮文言確定 / compound 発火例 / 失敗を隠さない原則 / 対象外カテゴリ将来方針 / 次タスク候補 (QUOTA-HANDLING / MASTER-V2-METADATA / AGGREGATION-TUNING / LEVEL-3-WARNING-BAR / INDICATOR-DETAIL)）
