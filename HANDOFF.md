...
**SAVE-UX 支部マスタ破損 root cause inventory**: `docs/notes/20260513_shogi_save_ux_status_map.md` §17（PR #87 後の follow-up docs-only 棚卸し。支部マスタの保存経路 16 callsite / 読み込み経路 26 callsite / `_loaded_with_corruption` 利用 4 callsite / import・migration 経路を整理。破損原因候補 10 件を「該当しそう（schema 不整合 / dev tools 操作 / 異常書込み）」「可能性低（quota / partial write / import 不正データ）」「判断不能（破損フラグ伝播 / 実運用ログ不足）」に分類。既存防御（normalizeBranchMaster / `_loaded_with_corruption` / quota 分岐 / resetAll で master 保持）と弱点（recovery guidance 欠 / 破損 raw 退避導線なし / callsite 全数 audit 未実施）を整理。次タスク候補は `SAVE-UX-BRANCH-MASTER-RECOVERY-GUIDANCE`（最有力、message 改善）/ `SAVE-UX-BRANCH-MASTER-CALLSITE-AUDIT`（docs-only）/ `SAVE-UX-STATE-RESTORE-HANDLING-INVENTORY` / `SAVE-UX-BRANCH-MASTER-CORRUPTION-RECOVERY-IMPL`）
