...
**SAVE-UX-QUOTA-HANDLING inventory (Step 1)**: `docs/notes/20260514_shogi_save_ux_quota_inventory.md`（quota / storage exception / save failure の棚卸し。docs-only。`localStorage.setItem` 2 callsite を確認、`QuotaExceededError` 明示判定は 0 件。暫定方針 `kind: 'storage-quota'` / `aggregateKey: 'storage-quota:global'` / `severity: 'warn'`、showMsg aggregation 対象外、indicator は同じ count に含める方向。**SAVE-UX-QUOTA-HANDLING-IMPL (Step 2) 着手前に必ず再仕様確認**）
