...
**SAVE-UX post-quota follow-up map**: `docs/notes/20260513_shogi_save_ux_status_map.md` §14（PR #78〜#80 後の到達点、save-verify / storage-quota / master-verify の **3 系統表**、未回収 Nice to Have、次タスク候補を整理。PR-A: SAVE-UX-MASTER-V2-METADATA-IMPL で S03 / S05 / S22 に master-verify metadata 付与済み。SAVE-UX-TEST-STRUCTURAL-MATCH で test を block 単位の structural match に移行済み（200 文字 window 依存解消）。後続候補は `SAVE-UX-MASTER-V2-AGGREGATION` (PR-B) ほか。`callsiteId` 命名規則は §13）
