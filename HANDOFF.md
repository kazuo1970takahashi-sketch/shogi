...
**SAVE-UX post-quota follow-up map**: `docs/notes/20260513_shogi_save_ux_status_map.md` §14（PR #78〜#80 後の到達点、save-verify / storage-quota / master-verify の **3 系統表**、未回収 Nice to Have 8 項目、次タスク候補を整理。PR-A: SAVE-UX-MASTER-V2-METADATA-IMPL で S03 / S05 / S22 に master-verify metadata 付与済み（aggregation 対象外、indicator 対象）。後続候補は `SAVE-UX-TEST-STRUCTURAL-MATCH`（足場固め）または `SAVE-UX-MASTER-V2-AGGREGATION` (PR-B)。`callsiteId` 命名規則は §13）
