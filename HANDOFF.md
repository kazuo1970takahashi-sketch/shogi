...
### 2026-05-12〜13: A-5.1 保存安全化（SAVE-001 / 002 / 003 / 004 / 003b-1 / 003b-2）
...
**残タスク候補**:
- A-5.1-SAVE-003b-3（手動編集系：`updateField` / `bulkEditNames`、SAVE-UX 集約方針合意が前提）
- A-5.1-SAVE-UX（warn 集約・retry UI・文言短縮）
- Security hygiene（`trufflesecurity/trufflehog@main` を SHA / tag 固定に変更、Mini Shai-Hulud 第二波 影響確認時に発見した別件、SAVE 系本流とは別 PR で扱う）
