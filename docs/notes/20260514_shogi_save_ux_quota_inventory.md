...
| 2026-05-14 | SAVE-UX-KIND-ASSERT-AND-CALLSITE-ID-DOCS (PR #79 Nice to Have 回収) に伴い §15.5 へ `callsiteId` 新形式 (`KIND-NAMESPACE:<funcName>`) を採用した旨を短く追記。詳細命名規則は status-map §13 で整理。 |
| 2026-05-14 | SAVE-UX-POST-QUOTA-FOLLOWUP-MAP (docs-only) に伴い §15.11 を追記。本書は quota 固有の詳細にスコープを限定し、PR #78〜#80 後の全体地図（save-verify / storage-quota 2 系統 / 未回収 Nice to Have / 次タスク候補）は status-map §14 へ集約することを明記。重複記載を避ける。 |
...
`callsiteId` は新形式 `KIND-NAMESPACE:<funcName>`（colon 区切り）を採用。値の hyphen-case 規則 (`kind` / `aggregateKey`) とは独立で、既存形式 (`S03` / `SAVE-XXX-funcName`) と併用する。詳細は `docs/notes/20260513_shogi_save_ux_status_map.md` §13 を参照。
...
本書は quota 固有の詳細（inventory / 実装 / 暫定方針）にスコープを限定する。PR #78〜#80 後の **全体地図**（save-verify / storage-quota の 2 系統状態、未回収 Nice to Have 8 項目、次タスク候補 8 件、次にやるなら）は重複記載を避けるため `docs/notes/20260513_shogi_save_ux_status_map.md` §14 に集約している。

- 次にやるなら（status-map §14.5）: 次候補は `SAVE-UX-MASTER-V2-METADATA`、足場固めなら `SAVE-UX-TEST-STRUCTURAL-MATCH`
