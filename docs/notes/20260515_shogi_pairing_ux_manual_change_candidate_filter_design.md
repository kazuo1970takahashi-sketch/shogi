...
<select id="chg-p1">
  ...
  <optgroup label="選択できない候補">
    ...
    <option value="..." disabled>A-21 鈴木次郎（同じ選手）</option>
...
| swap 経路（候補 X が他ペアに属す）| **disabled + 理由**（hard block と整合）| `A-18 佐藤一郎（再戦になる）` を選択不可 optgroup に置く |
...
| `R-rematch-swap` | （再戦になる）| swap 経路 + `pairHasRematch` 命中 | 事後判定（[L4778](shogi_v4.html:4778)）|
...
5. `R-rematch-swap`（再戦になる、hard block）
...
（選択できない候補）
A-21 鈴木次郎（同じ選手）     ← R-self（実装上の事前判定）
A-25 田中三郎（結果入力済）   ← R-winner-locked
A-30 中村四郎（再戦になる）   ← R-rematch-swap
```
...
- 理由ラベル（`（選ぶと再戦）` / `（再戦になる）` / `（結果入力済）` / `（同じ選手）` 等）が含まれる
...
| replace / swap の非対称が分かりにくい | 本 docs §6 で明文化。UI 上は理由ラベル `（選ぶと再戦）` / `（再戦になる）` で文言を分ける |
...
2. **`buildChangePairingModalHtml` を optgroup 2 段に刷新**
   ...
   - 理由ラベルは `（結果入力済）` / `（再戦になる）` / `（同じ選手）` の 3 種類（8〜12 文字、PR #110 で運営者向け文言に修正）
...
- 「同じ選手」/「再戦になる」が事前 disabled で表示される頻度（事後 alert への到達回数の減少）
- 候補 0 人案内の発動頻度
