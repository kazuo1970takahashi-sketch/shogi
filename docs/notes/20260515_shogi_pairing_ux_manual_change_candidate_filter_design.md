...
|---|---|---|---|
| `R-current` | （現在の相手）| そもそも変更対象なので並べない方針も可 | — |
...
- `classifyChangePairingCandidate(cls, idx, candidateId, role)` を直接呼び、以下を検証:
  - **R-self**: 候補 == match.p1（role='p1'）→ 現在の選手 / `ok` または `R-current` 扱い（仕様確定要）
  - **R-winner-locked**: 候補が結果入力済ペアに属する → `blocked` + 理由 `R-winner-locked`
  - **R-rematch-replace**: 候補が他ペアに属さず、再戦 → `warn-confirmable` + 理由 `R-rematch-replace`
  - **R-rematch-swap**: 候補が他ペアに属し、入替後再戦 → `blocked` + 理由 `R-rematch-swap`
  - 正常: `ok`
