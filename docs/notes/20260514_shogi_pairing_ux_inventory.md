...
### 3.7 警告と判断材料の画面距離（現場観察、2026-05-15 追補）

#### 観察された事象

大会運営中に、組み合わせ作成直後の画面で以下の **要確認メッセージ** が表示された:

> ⚠ 要確認：同勝数で組めた可能性のある異勝数ペア 1 件
> ※ 操作はブロックされません。例外判断は現場運営に委ねます。

これは `evaluatePairingQuality()` ([4384](shogi_v4.html:4384)) が算出する `avoidableWinDiffPairs`（[4437-4483](shogi_v4.html:4437)）が 1 以上のときに `buildCurrentPairingsHtml()` ([4868](shogi_v4.html:4868)) が表示する警告（[4885 / 4887](shogi_v4.html:4885)）。

#### 画面距離の問題

`renderTournament()` ([4961](shogi_v4.html:4961)) のレンダリング順:

| 順 | 関数 | 内容 | 画面位置 |
|---|---|---|---|
| 1 | `buildScoreGridHtml()` ([4822](shogi_v4.html:4822)) | **暫定成績**（各プレイヤーの勝敗数 score-card）| 上部 |
| 2 | `buildPlayedHistoryHtml()` ([4833](shogi_v4.html:4833)) | 対戦履歴 | 中部 |
| 3 | `buildPastResultsHtml()` | 過去結果 | 中下部 |
| 4 | `buildCurrentPairingsHtml()` ([4868](shogi_v4.html:4868)) | **現ラウンド組み合わせ + ⚠️警告バナー** | 下部 |

→ **警告は画面下部、勝敗数は画面上部** という構造。22 名クラス + 複数ラウンド進行時は **スクロールしないと突き合わせできない**。

#### 課題の整理

| 観点 | 現状 | 論点 |
|---|---|---|
| 異勝数ペア警告の判断材料 | 「1 件」とだけ表示。どのペアが該当かは `pairDetails[i].labels` の `[要確認]` ラベル（[4495-4496 / 4900](shogi_v4.html:4495)）でしか分からない | **警告本文に該当ペアの情報が無い** |
| 警告と暫定成績の画面距離 | 暫定成績は最上部、警告は最下部 | スクロール往復が必須 |
| 勝敗数の即時確認 | `getWins(cls)` の結果は score-grid にしか出ない | **ペアカード自体に勝敗数が併記されていない** |
| 警告の妥当性判断 | 「同勝数で組めた可能性がある」と判断した根拠（同勝群内で再戦回避マッチングが成立する候補）は内部計算（[4437-4483](shogi_v4.html:4437)）でしか持たず、画面に出ない | 運営者が「本当に避けられたのか?」を即判断できない |
| 該当ペアへのナビゲーション | `[要確認]` ラベルは付くが、警告バナー → 該当ペアへのジャンプリンクは無い | クラス内ペアが多いほど目視で探す負荷が高い |
| 同勝数で組めた候補の具体名 | `evaluatePairingQuality()` の内部で計算するが返却しない | **「A-12 と組めばよかった」という具体名が出れば判断は容易**、ただしロジック改修が必要 |

#### 整理すべき判断軸

- 組み合わせ結果（ペアカード）側に **勝敗数を併記** すべきか
  - 例: `A-12｜佐藤 (2勝1敗) ── A-03｜田中 (3勝0敗)` のように行内表示
- **要確認メッセージ内に該当ペアの勝敗数を表示** すべきか
  - 例: `異勝数ペア 1件: A-12｜佐藤 (2勝) vs A-03｜田中 (3勝)`
- 該当ペアの **ハイライト / ジャンプ** リンクを警告バナーに付けるべきか
  - クリックで該当ペアカードへ scroll、または背景色で強調
- 「同勝数で組めた可能性がある」と判断した **根拠候補（具体名）** を表示すべきか
  - ロジック改修必要、`evaluatePairingQuality()` の戻り値に「組めた候補」を追加

#### 重要

これは **観察された実例** であり、§3.1〜§3.6 のように事前ヒアリング由来の論点とは別軸の **現場気づき** である。警告自体は既に実装済み（A-4 §1, §6 の品質評価）だが、**警告の妥当性を運営者がその場で判断する材料が画面構造的に離れている** ことが課題。
...
| 項目 | 対応案 | 由来 |
|---|---|---|
| 同姓識別表示 | 「変更」モーダル option label に **クラス + 番号 + 苗字 + 支部** を含める / 行内表示も同様 | §3.1 |
| 候補者検索 / 絞り込み | `<select>` を search input + filter list に置換（スマホでもキーボードに頼らず指タップ）| §3.2 |
| 並び替え（コンテキスト依存）| 「未割当者を上」「同勝同士をグループ化」「対戦済みを下に」のソートトグル | §3.2 |
| 未割当者一覧 | 現ラウンドのペア外プレイヤーを **明示リスト** で常時表示 | §3.2 / §3.4 |
| 選択済み視認性 | 既にペアに入っている候補を **disabled / 別配色** で表示 | §3.2 |
| 「対戦済み」マーカー | モーダル候補 option に `(済)` アノテート / 行内表示で hover tooltip | §3.2 |
| 「全勝」マーカー | option label / 行内表示で `★全勝` 等のラベル | §3.3 |
| 印刷前最終確認 view | `submitRound()` の前段に「これで確定しますか?（未割当者 0 名 / 再対戦 0 / 重複 0 を表示）」step | §3.6 |
| **組み合わせ結果に勝敗数併記** | ペアカード行内に各プレイヤーの勝敗数を表示（例: `A-12｜佐藤 (2勝1敗)`）| **§3.7** |
| **要確認メッセージに該当ペア情報を含める** | `quality.avoidableWinDiffPairs` の警告本文に **該当ペア名 + 勝敗数** を追記 | **§3.7** |
| **該当ペアのハイライト** | 警告対象ペアカードに背景色 / アイコン強調を付与 | **§3.7** |
| **暫定成績へのジャンプリンク** | 警告バナーに「暫定成績を確認」リンク → score-grid へ scroll | **§3.7** |

### 4.2 B. ロジック改善が必要なもの

| 項目 | 対応案 | 由来 |
|---|---|---|
| 全勝者同士の警告 | `evaluatePairingQuality()` に「全勝者同士」判定を追加し `[全勝対戦]` ラベルを返す（**禁止ではなく警告**）| §3.3 |
| デッドロック予兆（軽量）| 「変更」モーダル open 時に「このペア変更は残り組合せを詰ませる可能性あり」の簡易チェック（候補数 / 残ペアの再対戦グラフ）| §3.4 |
| 残人数・候補ペア妥当性 | `submitRound()` の事前 validation を強化（現状の alert より前に画面で表示）| §3.4 |
| 対戦済みグラフ視覚化 | 横断 view（既存 `buildPlayedHistoryHtml()` [4833](shogi_v4.html:4833) を発展）| §3.4 |
| **同勝数で組めた候補の具体名表示** | `evaluatePairingQuality()` の戻り値に `avoidableWinDiffDetails`（該当ペア + 「組めたはずの相手」候補名）を追加 | **§3.7** |
| **異勝数ペア警告の根拠表示** | 警告文に「A-12（2勝）vs A-03（3勝）が同勝群内 A-12 vs A-08（2勝）にできた可能性あり」のような **判定根拠** を含める | **§3.7** |

### 4.3 C. 運用ルールで吸収すべきもの

| 項目 | 運用ガイド明文化候補 | 由来 |
|---|---|---|
| 全勝者同士を避けるか | 「最終戦前は避ける / それ以外は許容」など、**ラウンド位置に応じた指針** | §3.3 |
| 紙カード配布後の訂正方法 | 「カード配布前は自由 / 配布後は途中棄権・誤記のみ訂正、ペア再構成はしない」 | §3.6 |
| 同姓同名時の呼び方 | 「クラス + 番号 + 苗字 + 支部」を口頭呼称の単位とする | §3.1 |
| 欠番許容 | `entry_no` の欠番は許容（途中棄権で詰めない）| §3.5 |
| 個人情報の取扱い | 運営者画面と公開画面を分ける運用 / 印刷物の取扱い手順 | §3.5 |
| **要確認警告が出た場合の対応手順** | 「警告が出たら **暫定成績を確認** して最終判断する。妥当と判断したらそのまま進行、避けたい場合は手動でペア変更」を運用手順に明文化。**将来的には UI 上で判断材料を近接表示** する（→ `WARNING-DECISION-SUPPORT`） | **§3.7** |
...
### 5.6 `PAIRING-UX-WARNING-DECISION-SUPPORT`（2026-05-15 追補）

- **目的**: 異勝数ペアなどの **要確認メッセージ** について、**警告の近くに判断材料を表示** し、運営者がスクロールせずに妥当性を確認できるようにする
- **由来**: §3.7 現場観察（「同勝数で組めた可能性のある異勝数ペア 1 件」警告が画面下部、判断に必要な勝敗数が画面上部の暫定成績にしかなく、スクロール往復が必須だった）
- **想定変更**:
  - **第 1 段（小）**: ペアカード行内に勝敗数併記 — `buildCurrentPairingsHtml()` ([4868](shogi_v4.html:4868)) で `getWins(cls)` の結果を `getNameWithNo()` 出力に追記
  - **第 2 段（中）**: 警告本文に該当ペア + 勝敗数を含める — `evaluatePairingQuality()` ([4384](shogi_v4.html:4384)) の戻り値に `avoidableWinDiffDetails` 配列を追加、警告レンダリング（[4885](shogi_v4.html:4885)）で展開
  - **第 3 段（中〜大）**: 警告 → 該当ペアへのハイライト / ジャンプ — DOM anchor + scrollIntoView
  - **第 4 段（大）**: 「組めたはずの相手」候補名を表示 — `evaluatePairingQuality()` の `avoidableWinDiffPairs` 算定（[4437-4483](shogi_v4.html:4437)）で見つけた候補を保持して返す
- **性格**: **docs-only design → UI small fix**（第 1〜第 3 段は表示層中心、第 4 段はロジック改修）
- **優先度**: **`PAIRING-UX-DISPLAY-LABELS` と並ぶ高優先度候補**
- **理由（高優先度の根拠）**:
  - **大会中に実際に確認負荷が発生** している（観察された実例）
  - 警告自体は既に実装済みだが、**判断材料の近接性が無い** ため警告の有効性が低下
  - 第 1 段だけでも体感改善が大きい（ペアカードに勝敗数併記）
  - `DISPLAY-LABELS` の表示 helper と共有可能（行内表示の拡張という意味で連携しやすい）
- **対象外（IMPL-LIGHT のスコープ非対象）**:
  - 警告判定ロジック自体の変更（`avoidableWinDiffPairs` の閾値変更等）— 現状ロジックは A-4 §1, §6 で確定済
  - 警告を block 化する（現状「※ 操作はブロックされません。例外判断は現場運営に委ねます。」を維持）
  - 自動再ペアリング（警告対象を自動で組み直す）

### 5.7 候補比較表

| 候補 | 性格 | 想定変更量 | 個人情報配慮 | 運用ルール影響 | 由来 |
|---|---|---|---|---|---|
| `DISPLAY-LABELS` | design → small UI | 小〜中 | あり（表示範囲整理）| 小 | §3.1 / §3.2 / §3.5 |
| `UNASSIGNED-VISIBILITY` | UI small fix | 小 | なし | 小 | §3.2 / §3.4 |
| `WINNER-CONFLICT-WARNING` | design check | 中（ロジック含む）| なし | **大**（ラウンド指針）| §3.3 |
| `DEADLOCK-INVENTORY` | docs-only | 0（docs のみ）| なし | 中 | §3.4 |
| `CARD-PUBLISH-CHECK` | design → small UI | 小〜中 | あり | 中 | §3.6 |
| **`WARNING-DECISION-SUPPORT`** | **design → UI small fix** | 段階: 小→中 | なし | 小（運用ガイド明文化のみ）| **§3.7** |
...
§3.1〜§3.7 のうち、**現場で即効性が高い 2 件** を高優先度に置く。両者は表示層中心で重ならず、表示 helper を共有できるため **並走可能**。

| 順位 | 候補 | 直結する課題 | 性格 |
|---|---|---|---|
| **第一候補 A** | `PAIRING-UX-DISPLAY-LABELS` | 同姓識別・大人数クラス選択負荷 (§3.1 / §3.2) | design → small UI |
| **第一候補 B** | `PAIRING-UX-WARNING-DECISION-SUPPORT` | 警告と判断材料の画面距離 (§3.7、現場観察済み) | design → UI small fix |

### 6.1 第一候補 A: `PAIRING-UX-DISPLAY-LABELS`
...
### 6.2 第一候補 B: `PAIRING-UX-WARNING-DECISION-SUPPORT`（2026-05-15 追補）

**理由**:
- **大会中に実際に確認負荷が発生** している（§3.7、現場観察済みの実例）
- 警告自体は既に実装済みだが、判断材料が画面の反対側にあるため **警告の有効性が低下** している
- 第 1 段（ペアカードに勝敗数併記）だけでも体感改善が大きい
- `PAIRING-UX-DISPLAY-LABELS` の表示 helper と **共有可能**（行内表示の拡張という意味で連携しやすい）
- 段階分けが明確（第 1 段: 勝敗数併記 → 第 2 段: 警告本文に該当ペア → 第 3 段: ハイライト・ジャンプ → 第 4 段: 候補名表示のロジック改修）
- ロジック変更なしの第 1〜第 3 段だけでも実装可能（小〜中規模）

### 6.3 第二候補: `PAIRING-UX-UNASSIGNED-VISIBILITY`

**理由**:
- 手動組み合わせ中の **迷い** を減らせる
- UI 小修正で効果が出やすい（state shape 変更なし）
- 第一候補（`DISPLAY-LABELS` / `WARNING-DECISION-SUPPORT`）の表示 helper を活用できる

### 6.4 第三候補: `PAIRING-UX-DEADLOCK-INVENTORY`
...
- `WARNING-DECISION-SUPPORT` 第 4 段（候補名表示）の即実装（第 1〜第 3 段の効果を観察してから判断）
- 警告判定ロジック自体の変更（`avoidableWinDiffPairs` の閾値等は現状維持）
