...
| **A-5.1-SAVE-003b-1**（追加） | 参加者追加経路の保存未確認検知 | S02（handlePastParticipantClassAdd state、追加・変更）/ S04（handleSuggestClassAdd state、追加・変更）/ S07（finalizeAddPastParticipants） | ✅ **完了・main マージ済**（PR #52、commit `af2f173`）。`verifyPlayerPersistedById(id, cls, name)` を流用、新 helper 追加なし。`handleSuggestClassAdd` は postSuccess に verify 引数追加、`handlePastParticipantClassAdd` は新規追加経路 / クラス変更経路の両方に verify、`finalizeAddPastParticipants` は addedTargets を保持し複数件未確認でも warn 1 回に集約 |
| **A-5.1-SAVE-003b-2**（追加） | 対局画面 編集経路の保存未確認検知（Must） | S17（bindChangePairingModalEvents）/ S19（bindEditPastResultModalEvents） | 🔲 未着手。SAVE-003b 設計（PR #51 §6）で次の小粒 PR と位置付け。`readPersistedState` + `pairingsMatchSnapshot` 流用 + インライン winner 比較で helper 追加なし想定 |
| **A-5.1-SAVE-003b-3**（追加） | 手動編集系の保存未確認検知（Should） | S08（updateField）/ S11（bulkEditNames） | 🔲 未着手。SAVE-UX の warn 集約方針合意が前提。新 helper `verifyPlayerFieldPersisted` の追加検討 |
