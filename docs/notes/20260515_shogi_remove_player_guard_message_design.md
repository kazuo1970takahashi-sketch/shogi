...
**Task ID**: `REMOVE-PLAYER-GUARD-MESSAGE-IMPL-LIGHT`
**Branch**: `feat/remove-player-guard-message-impl-light`
**設計 PR**: PR #115（squash merge `059ccdb`）
**実装 PR**: 本タスクの Draft PR

### 17.1 採用した実装位置

[`removePlayer()`](../../shogi_v4.html) ([L3761-3809](../../shogi_v4.html)) 内の **2 つの alert 文言** をそれぞれ §6.4 確定文に置換:

- 一次禁止 alert: 旧 L3769 → 新 alert（in `if(inPairings)` ブロック）
- 二次禁止 alert: 旧 L3781 → 新 alert（in `if(state.started && pastMatches>0)` ブロック）

**判定ロジック / early return 順序 / 削除成功経路はすべて温存**（文字列リテラルの差し替えのみ + 各 if ブロック直前に解説コメント追加）。

### 17.2 採用した一次禁止 alert 文言
...
§6.4.1 第一候補をそのまま採用（短縮版ではなく full 版）。理由:
- 「対戦相手変更」を **第一案** として明示することで既存 soft recovery を温存
- 「大会進行データをリセット」を **第二案** として併記
- 「参加者一覧は残したまま」を明示
- PR #114 で更新済の [`startTournament()`](../../shogi_v4.html) guard alert と語彙整合

### 17.3 採用した二次禁止 alert 文言
...
§6.4.2 第一候補をそのまま採用。理由:
- 「対戦履歴」→「勝敗結果」用語統一（`state.results` 由来であることが直感的）
- 「大会開始後は削除できません」削除（state.started 状態の言及は実装詳細寄り）
- 「リセット」→「大会進行データをリセット」具体化、`#resetBtn` 誤押下抑止
- 「参加者一覧は残したまま」明示
- `pastMatches+'試合'` 動的文字列を維持（運営者が件数を把握できる）

### 17.4 既存挙動を維持したもの

- **`removePlayer()` 判定ロジック**:
  - 一次禁止 `if(inPairings)`（`state.pairings[cls]` メンバーシップ判定）変更なし
  - 二次禁止 `if(state.started && pastMatches>0)` 変更なし
  - `pastMatches` の `state.results[cls]` 走査による算出 変更なし
- **early return 順序**: 一次禁止 → 二次禁止 → confirm 経由削除成功（順序維持）
- **削除成功経路**: `state.players[cls]=arr.filter(...)` / `delete _pendingNewYomi[id]` / `renderRegList()` / `save()` / `verifyPlayerAbsent` / `notifySaveWarning` すべて変更なし
- **関連機能**: [`resetTournamentProgressOnly()`](../../shogi_v4.html) / [`resetAll()`](../../shogi_v4.html) / [`startTournament()`](../../shogi_v4.html) guard 条件 / `normalizeState` / localStorage schema / pairing algorithm 変更なし
- **ボタン文言**: 既存 `#resetBtn`「大会データをリセット」変更なし / 既存 `#resetProgressBtn`「大会進行データをリセット」変更なし
- **confirm 文言**: `resetAll()` confirm「現在の大会データをリセットします（支部マスタは保持されます）」変更なし / `resetTournamentProgressOnly()` confirm「参加者一覧は残したまま、現在の組み合わせ・勝敗結果を削除します。\nよろしいですか？」変更なし
- 既存 helper 群（`getName` / `getNameWithNo` / `entryNoOf` / `formatParticipantLabel` / `pairHasRematch` / `findPairContainingPlayer` / `getDuplicatePlayersInPairings` / `classifyChangePairingCandidate`）変更なし

### 17.5 両方該当ケースの caveat（§6 で予告した内容）

参加者が **pairings + results 両方** に該当する場合（例: R2 進行中、R1 に勝敗結果あり、R2 ペアにも含まれる）:

- 既存 early return 順序により **一次禁止 alert が先に発火**（二次禁止には到達しない）
- 運営者が「対戦相手変更」で R2 pairings から外すと、次回の `removePlayer` 呼出では一次禁止をすり抜け、**二次禁止 alert へ進む**（R1 results が残っているため）
- → 一次禁止解消後に二次禁止が出る場合がある旨を、運営者は本 design doc / 実装着地ノートで認知できる
- 自動的に reset まで連鎖させる挙動は **案 E**（IMPL-MEDIUM 候補）として後送り

本 IMPL-LIGHT は順序を変更せず、文言更新のみで対応（PR #115 §6 / §7.1 / §11 方針通り）。

### 17.6 追加テスト

新規ファイル: [`test/test_remove_player_guard_message.js`](../../test/test_remove_player_guard_message.js)

`test/run_tests.sh` に呼出 stanza を追加。

#### 17.6.1 静的検査の主要観点（PASS 69 件）

**一次禁止 alert 文言**:
- 「現在の組み合わせ」「削除できません」「対戦相手変更」「大会進行データをリセット」「参加者一覧は残したまま」「組み合わせ・勝敗結果」含有
- 「大会データをリセット」(#resetBtn 文言) を含まない（誤誘導抑止、`-大会進行データをリセット` 除外後検査）
- 「リセット」単独表現を含まない

**二次禁止 alert 文言**:
- 「勝敗結果」「削除できません」「大会進行データをリセット」「参加者一覧は残したまま」「組み合わせ・勝敗結果」含有
- `pastMatches+'試合'` 動的文字列維持
- 「大会データをリセット」(#resetBtn 文言) を含まない / 「リセット」単独表現を含まない

**判定ロジック不変**:
- 一次禁止 `if(inPairings)` / 二次禁止 `if(state.started && pastMatches>0)` 維持
- pastMatches の state.results 走査 + p1/p2 判定 維持
- 一次禁止が二次禁止より前にある（early return 順序）

**削除成功経路不変**:
- `var msg=name+'を削除しますか'` confirm 文言維持
- `if(!confirm(msg))return` キャンセル時 return 維持
- `state.players[cls]=arr.filter(p => p.id!==id)` 維持
- `delete _pendingNewYomi[id]` 維持
- `renderRegList()` / `save()` / `verifyPlayerAbsent` / `notifySaveWarning` + `callsiteId:'SAVE-001-removePlayer'` 維持

**関連機能不変**:
- `resetTournamentProgressOnly()` / `resetAll()` 本体維持（players 初期化 / confirm 文言）
- `startTournament()` guard 条件 `state.started===true` + PR #114 同期更新済 alert 文言維持
- `#resetProgressBtn` / 既存 `#resetBtn` 文言維持
- localStorage schema (`STORAGE_KEY` / `LEGACY_STORAGE_KEYS` / `BRANCH_MASTER_KEY`) 維持
- pairing algorithm (`generatePairing` / `evaluatePairingQuality`) 維持

**両方該当ケース**:
- 一次禁止 alert の文言と二次禁止 alert の文言が双方存在し区別可能
- 一次禁止 alert の後に `return;` が存在（二次禁止に到達しない）

### 17.7 既存テスト更新

PR #114 IMPL-LIGHT 時に書かれた **旧文言を assertion している 3 箇所** を、新文言（PR #116 案 D）に追従更新:

1. [`test/test_reset_ux_partial_reset.js:243-246`](../../test/test_reset_ux_partial_reset.js): 「進行中の対局に登録されているため削除できません」→「現在の組み合わせに登録されているため削除できません」/ 「試合分の対戦履歴があるため、大会開始後は削除できません」→「試合分の勝敗結果があるため削除できません」
2. [`test/test_reception_ux_start_button_guard.js:225-228`](../../test/test_reception_ux_start_button_guard.js): 同上
3. [`test/run_tests.sh:109`](../../test/run_tests.sh): `grep -q "進行中の対局に登録されているため削除できません"` → `grep -q "現在の組み合わせに登録されているため削除できません"`

これらは **テスト logic の本体は変えず**、期待文言のみ追従更新（PR #114 IMPL-LIGHT のときに `test_reception_ux_start_button_guard.js` を PR #112 alert 更新に追従したのと同じパターン）。
...
```
結果: PASS=76, FAIL=0, WARN=0
✓ 全テスト合格(警告: 0件)
```

既存 75 + 新規 1 = **全 76 stanza PASS / FAIL=0 / WARN=0**。

### 17.9 VRT / E2E

- alert dialog は overlay で表示され、Playwright の `toHaveScreenshot` には通常含まれない → snapshot mismatch 見込みなし
- 既存 E2E が `removePlayer` alert 文言を `toContainText` 等で expect している場合は追従更新が要 → `grep -r '進行中の対局\|対戦履歴があるため' test/e2e/` で事前確認した結果、該当なし（影響なし見込み）
- 万一 red になった場合は **自律 snapshot 更新せず判断を仰ぐ**

### 17.10 やっていないこと（IMPL-MEDIUM 以降の余地）

- `removePlayer()` 判定ロジック変更
- 自動部分リセット連鎖（案 E）
- 専用モーダル化（`REMOVE-PLAYER-MODAL-DESIGN` 候補）
- UI ボタン追加
- `resetTournamentProgressOnly()` / `resetAll()` 仕様変更
- `startTournament()` guard 条件変更
- `changePairing` モーダル変更
- localStorage schema / pairing algorithm / `state.results` 構造変更
- 参加者番号再採番
- 大会履歴保存 / undo
- 既存 `#resetBtn` / `#resetProgressBtn` 文言変更
- 既存 confirm 文言変更
