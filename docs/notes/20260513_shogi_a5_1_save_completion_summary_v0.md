...
- 作成日: 2026-05-13
- 改訂日: 2026-05-13（SAVE-004 / SAVE-003b-1 完了反映）
- 位置づけ: A-5.1-SAVE-001 / SAVE-002 / SAVE-003 / SAVE-004 / SAVE-003b-1 の完了状況を 1 ファイルに集約した整理メモ
...
|---|---|---|---|
| **A-5.1-SAVE-003b-2** | 対局画面 編集経路の保存未確認検知（Must） | S17（bindChangePairingModalEvents 対戦相手変更 / swap）/ S19（bindEditPastResultModalEvents 過去結果修正） | SAVE-003b 設計（PR #51 §6）で次の小粒 PR と位置付け。`readPersistedState` + `pairingsMatchSnapshot` 流用 + インライン winner 比較で helper 追加なし想定。**未着手** |
...
注: タスク ID `SAVE-003b-2` / `SAVE-003b-3` / `SAVE-UX` は本ドキュメント時点での暫定名。正式起票時に再採番してよい。`SAVE-004` / `SAVE-003b-1` は 2026-05-13 に完了済（§1 参照）。
