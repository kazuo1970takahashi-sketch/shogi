...
- 作成日: 2026-05-13
- 改訂日: 2026-05-13（SAVE-004 / SAVE-003b-1 / SAVE-003b-2 完了反映）
- 位置づけ: A-5.1-SAVE-001 / SAVE-002 / SAVE-003 / SAVE-004 / SAVE-003b-1 / SAVE-003b-2 の完了状況を 1 ファイルに集約した整理メモ
...
|---|---|---|---|
| **A-5.1-SAVE-003b-3** | 手動編集系の保存未確認検知（Should） | S08（updateField）/ S11（bulkEditNames） | SAVE-UX の warn 集約方針合意が前提。新 helper `verifyPlayerFieldPersisted` の追加検討。**未着手** |
...
| **Security hygiene**（A-5.1 とは別系統） | `trufflesecurity/trufflehog@main` を SHA / tag 固定に変更（Mini Shai-Hulud 第二波 影響確認時に発見、IoC とは別件の一般 supply-chain hygiene） | `.github/workflows/e2e.yml` | **未着手**。SAVE 系の本流とは別 PR で扱う。今回の docs PR には含めない |
...
注: タスク ID `SAVE-003b-3` / `SAVE-UX` / Security hygiene は本ドキュメント時点での暫定名。正式起票時に再採番してよい。`SAVE-004` / `SAVE-003b-1` / `SAVE-003b-2` は 2026-05-13 に完了済（§1 参照）。
