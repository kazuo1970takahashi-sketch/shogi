...
| 5 | 200 文字 window → structural match | test pair count が 200 文字 window 依存。将来整形やコメント追加で false negative になり得る | `SAVE-UX-TEST-STRUCTURAL-MATCH` | 中（次の kind 追加前か同時） |
...
| 2 | `SAVE-UX-MASTER-V2-METADATA` | MASTER-V2-LASTCLASS S03 / S05 / S22 を `master-verify` 系として metadata 化するか検討 | save-verify / storage-quota の 2 系統が安定した後 | **高** | 次の主要候補。SECTION 16 以降の kind 別 assert 追加テンプレートにもなる |
...
| 7 | `SAVE-UX-TEST-STRUCTURAL-MATCH` | 200 文字 window の pair count を `notifySaveWarning` block 単位の structural match に置き換える | 次の kind 追加前、または test 整理 PR 時 | 中 | 実装ロジックには影響しない test 改善 |
| 8 | `SAVE-UX-KIND-ASSERT-TABLE` | kind 別 expected count / aggregateKey 分布をテーブル化 | 3 系統以上になった時点 | 低〜中 | 今すぐは不要。SECTION 方式で十分 |

### 14.5 次にやるなら

#### 1. すぐ実装に行くなら: `SAVE-UX-MASTER-V2-METADATA`

**理由**: save-verify / storage-quota の 2 系統が安定したため、次の kind 追加候補として自然。MASTER-V2-LASTCLASS S03 / S05 / S22 は既に対象外として何度も言及されており、整理対象として見通しがよい。本書 §14.4 でも推奨度 **高**。

#### 2. 先に足場を固めるなら: `SAVE-UX-TEST-STRUCTURAL-MATCH`

**理由**: 次の kind 追加前に 200 文字 window 依存を structural match に置き換えると、test の保守性が上がる。SECTION 16 以降の kind 追加時に false negative リスクを下げる。

#### 3. 実運用フィードバックを待つなら: `SAVE-UX-DUAL-NOTIFY-SUPPRESSION` / `SAVE-UX-AGGREGATION-TUNING`

**理由**: 二重通知や alert 連発は、実際にうるさいかどうかを見てから判断してよい。実運用フィードバックを得てから着手するのが安全。

#### 司令塔おすすめ

次は **`SAVE-UX-MASTER-V2-METADATA`** の仕様確認に進む。ただし、その前に test 改善を挟むなら **`SAVE-UX-TEST-STRUCTURAL-MATCH`** も可。
