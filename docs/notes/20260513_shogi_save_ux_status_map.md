...
**理由**: 200 文字 window 依存を structural match に置き換えると、test の保守性が上がる。SECTION 16 が追加された現時点でも window 依存は残るため、kind 追加が続くなら早めに着手するメリットあり。
...
PR-A 完了時点では、次は **`SAVE-UX-TEST-STRUCTURAL-MATCH`**（足場固め）または **`SAVE-UX-MASTER-V2-AGGREGATION`** （PR-B）が候補。実運用フィードバック次第で `SAVE-UX-DUAL-NOTIFY-SUPPRESSION` / `SAVE-UX-AGGREGATION-TUNING` も候補に上がる。
