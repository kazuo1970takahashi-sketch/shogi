...
| 外部送信・ログ送信 | §18.4.1 設計方針 / §18.8.1 個人情報の取扱い（#4「破損データを diagnostic で外部送信する案は採用しない」端末内完結前提）|
...
## 23. SAVE-UX-CLOSURE-DOC-REFINEMENT（v2.3 追補 / データ保全系 inventory 完了整理）

- 種別: **docs-only refinement**（実装はしない / §11〜§18 / §20〜§22 を改訂しない / §19.5 のみ Nice to Have 取込で微修正）
- 対象 main HEAD: `5cd4cbb`（PR #94 state restore inventory squash merge 後）
- Task ID: `SAVE-UX-CLOSURE-DOC-REFINEMENT`
- 目的: SAVE-UX データ保全系の inventory が §17〜§22 まで揃った時点での **closure refinement**。Codex PR #91 Nice-to-Have（§19.5 ↔ §18.8.1 併記）の取込 + §17〜§22 到達点の整理 + 次フェーズ判断の明示を行う。新規 inventory は作らない（次は impl 起票判断、または観察フェーズ）
- 起源: §20.5.3 / §21.11.3 / §22.8.3 で「Nice to Have」「単独 or 後続」として推奨された微小タスクの着地

### 23.1 本 refinement の目的

1. PR #91 Codex Nice-to-Have の取込: §19.5「外部送信・ログ送信」非実装根拠に §18.8.1 を併記し、追跡性を改善（本 PR で §19.5 を 1 箇所だけ更新）
2. §17〜§22 到達点を 1 ページで俯瞰できる closure を提供（個別 § を読まずに「どこまで終わっているか」を把握）
3. 残実装候補を 1 表に集約し、優先順位と着手条件を明示
4. 「次は何をするか」を司令塔判断材料として整理（新規 docs-only inventory に進まないことを宣言）

非目的:
- 新規 inventory / audit / 設計の追加
- §17〜§22 の本文改訂（§19.5 1 行のみ Nice to Have 取込）
- 後続 impl タスクへの着手
- 個別 § の数値や結論の再評価（既存の数値はすべて維持）
...
| § | PR | 種別 | 到達点 |
|---|---|---|---|
| §17 | [#88](https://github.com/kazuo1970takahashi-sketch/shogi/pull/88) `9d688c4` | docs | root cause inventory 完了。破損原因候補 10 件 / 既存防御 / 弱点 10 件 / 復旧導線候補 8 件 |
| §18 | [#89](https://github.com/kazuo1970takahashi-sketch/shogi/pull/89) `725941a` | docs | recovery guidance 設計完了。Light / Medium / Heavy 分類、message 案 L-1〜L-3 |
| §18.5.1 | [#90](https://github.com/kazuo1970takahashi-sketch/shogi/pull/90) `80c6cfe` | 実装 | PARSE-MASTER-003 IMPL-LIGHT 完了。「マスタタブのインポート・統合で復旧できます」追加 |
| §19 | [#91](https://github.com/kazuo1970takahashi-sketch/shogi/pull/91) `225acf8` | docs | closure 完了。到達点 + 維持されている方針 + 非実装リスト + 後続候補 |
| §21 | [#93](https://github.com/kazuo1970takahashi-sketch/shogi/pull/93) `f829009` | docs / audit | callsite audit 完了。`loadBranchMaster()` 24 callsite × `_loaded_with_corruption` 判定マトリクス、§17.9 弱点 #8 を「監査済」へ遷移 |

**残り**: addPlayer yomi 補完経路（[shogi_v4.html:3613→3618](shogi_v4.html:3613)）に中リスク 1 件を特定（§21.5.1）。即実装ではなく `SAVE-UX-ADDPLAYER-CORRUPTION-GUARD` として後続化、運用フィードバック後に優先度再判断（§21.11.2）。

#### 23.2.2 state restore 側

| § | PR | 種別 | 到達点 |
|---|---|---|---|
| §22 | [#94](https://github.com/kazuo1970takahashi-sketch/shogi/pull/94) `5cd4cbb` | docs | inventory 完了。PARSE-LOAD-001 / 002 / 003 を HEAD `f829009` で再検証、3 callsite すべて完全 silent と確認 |
| §22.3.3 | 同上 | docs（細則）| PARSE-LOAD-002 を 002A（v4→v3 成功 = 古いデータ起動）/ 002B（v4→v3 共に失敗 = 003 合流）にサブ分類 |
| §22.5.3 | 同上 | docs（severity）| PARSE-LOAD-003 の severity 昇格候補（warn 案 vs error 案）を整理 |
| §22.7.0 | 同上 | docs（timing 論点）| boot 経路と indicator UI 初期化の発火 timing 差を論点化、IMPL-LIGHT 着手時の選択肢 4 案 |

**残り**: 実装は **本 closure 時点で未着手**。LIGHT / 案 B / FULL の 3 段階（§22.7.2）に整理済。

### 23.3 docs-only inventory pool の状態

§20.5 推奨 Next Action 3 候補（PR #92 で起票）の着地状況:

| § | 推奨 Task ID | 着地状況 |
|---|---|---|
| §20.5.1 第一推奨（Must）| `SAVE-UX-BRANCH-MASTER-CALLSITE-AUDIT` | ✅ [#93](https://github.com/kazuo1970takahashi-sketch/shogi/pull/93) / §21 で完了 |
| §20.5.2 第二推奨（Should）| `SAVE-UX-STATE-RESTORE-HANDLING-INVENTORY` | ✅ [#94](https://github.com/kazuo1970takahashi-sketch/shogi/pull/94) / §22 で完了 |
| §20.5.3 第三推奨（Nice to Have）| `SAVE-UX-CLOSURE-DOC-REFINEMENT` | ✅ **本 PR（§23）で完了予定** |

**結論**: SAVE-UX **データ保全系の主要 docs-only inventory pool は本 PR 着地で一区切り**。次は実装候補の起票判断、または運用観察フェーズへ移行する。**新規 docs-only inventory を探し続けない**（後述 §23.5）。

### 23.4 残実装候補（一覧）

§21.10.2 / §22.7.2 の各案を 1 表に集約。本 closure では着手せず、起票判断は司令塔判断 1 回で可能な状態。

#### 23.4.1 支部マスタ系

| Task ID 案 | 性格 | 範囲 | 根拠 | 判断 |
|---|---|---|---|---|
| `SAVE-UX-ADDPLAYER-CORRUPTION-GUARD` | 実装（小）/ 運用フィードバック後 | [shogi_v4.html:3613→3618](shogi_v4.html:3613) `addPlayer` yomi 補完経路 | §21.10.2 / §21.11.2 | すぐ起動しない。中リスク 1 件を切り出して観察フェーズに乗せた状態（§21.11.2）、運用観察で実害が観測された場合に再判断 |
| `SAVE-UX-MODIFY-EXISTING-CORRUPTION-GUARD` | 実装（中）| 注目 6 callsite すべて（§21.4 read+save かつ flag 未読の 6 件）に flag 検知を横展開 | §21.10.2 | `ADDPLAYER-CORRUPTION-GUARD` より後、横展開判断あり |
| `SAVE-UX-BRANCH-MASTER-FLAG-COVERAGE` | 実装（中〜大）| `normalizeBranchMaster` schema 不一致パスでも flag を立てる拡張、§17.9 弱点 #10 と結合 | §21.6.2 / §21.10.2 | すぐ起動しない。schema_version bump policy と同時設計が必要、設計判断重め |

#### 23.4.2 state restore 系

| Task ID 案 | 性格 | 範囲 | 根拠 | 判断 |
|---|---|---|---|---|
| `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT`（仮）| 実装（小）/ 条件付き | PARSE-LOAD-003 のみ warn 化 | §22.7.2 / §22.8.3 | **次の実装候補として最有力**。ただし §22.7.0 boot timing / indicator UI 初期化論点を事前確認 |
| `SAVE-UX-STORAGE-CORRUPTED-HANDLING-IMPL` | 実装（中）/ 条件付き | PARSE-LOAD-002 + PARSE-LOAD-003 | §16.9 案 B / §22.7.2 | LIGHT の上位案。002A / 002B の UX 差（§22.3.3）を含めるならこちら |
| `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-FULL`（仮）| 実装（中〜大）| PARSE-LOAD-001 + PARSE-LOAD-002 + PARSE-LOAD-003 | §22.7.2 | 現時点では重い。LOAD-001（`getItem` 例外）の発火頻度が低いなら後回し |

### 23.5 次フェーズの推奨

#### 23.5.1 第一推奨: `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT`（仮）

理由:
- state restore は 3 件すべて完全 silent（§22.3.2）で、観察だけでは検出が極めて困難
- LIGHT なら PARSE-LOAD-003 のみで小さく始められる（§22.7.2 第一案）
- 「全キー失敗 = 大会データ全消失」の致命点をまず可視化する価値が高い
- ただし §22.7.0 boot timing / indicator UI 初期化論点を **事前に解く必要がある**（IMPL のスコープ膨張防止）

#### 23.5.2 第二推奨: `SAVE-UX-STORAGE-CORRUPTED-HANDLING-IMPL`（§16.9 案 B 既出）

理由:
- PARSE-LOAD-002 + 003 をまとめて扱える
- §16.9 案 B と一致、§16 既存設計の自然な拡張
- ただし v4 失敗→v3 成功の **PARSE-LOAD-002A UX**（§22.3.3、古いデータで起動するシナリオ）まで含むため、LIGHT より設計判断が重い
- LIGHT 着地後に検討するのが順序として自然

#### 23.5.3 第三推奨: 運用観察フェーズ継続

理由:
- 支部マスタ側は PR #90 IMPL-LIGHT 着地済（PARSE-MASTER-003 復旧導線、§18.5.1 L-1）
- `addPlayer` 中リスク 1 件は後続化済み（§21.11.2、運用フィードバック観察期間中）
- すぐに全実装を進めず、現場フィードバックを待てる
- 観察フェーズが「動かない」のではなく「観察 + impl 起票判断の併走」を意図

#### 23.5.4 明示: 次に docs-only inventory を探し続けない

本 §23 着地後の方針:

- ✅ 次は **impl 起票判断**（推奨は `STATE-RESTORE-HANDLING-IMPL-LIGHT`）または **観察フェーズ**
- ❌ 新規 docs-only inventory の追加を起動しない（pool は本 PR で一区切り）
- ❌ §17〜§22 の追加改訂を起動しない（既存記録の参照価値は維持）
- 例外: 大会運営現場での実害観測 / impl 着手時に発見された新規論点があれば、その時点で新規 §24+ を検討する

### 23.6 PR #91 Nice to Have 取込（§19.5 微修正）

本 PR で同時に対応した小さな refinement:

- §19.5 「外部送信・ログ送信」非実装行に **§18.8.1 個人情報の取扱い #4**（「破損データを diagnostic で外部送信する案は採用しない」「端末内完結が前提」）への参照を追加
- 既存の §18.4.1 参照と併記する形で、追跡性を改善
- §18.4.1 = 設計方針（recovery guidance の禁則）、§18.8.1 = 個人情報取扱い（外部送信不採用根拠）の **2 観点から trace 可能** な状態にする
- Codex PR #91 Nice to Have 指摘の取込完了

### 23.7 関連 docs / コード

- `docs/notes/20260513_shogi_save_ux_status_map.md`
  - §17（支部マスタ root cause）/ §18（recovery guidance 設計）/ §18.4.1 / §18.5.1 / §18.8.1
  - §19（branch master closure）/ §19.5（本 PR で §18.8.1 参照追加）
  - §20（next inventory）/ §20.5（推奨 Next Action 3 候補、本 PR で全件着地）
  - §21（callsite audit）/ §21.10.2（実装最小スコープ 3 案）/ §21.11.2 / §21.11.3
  - §22（state restore inventory）/ §22.7.0（boot timing 論点）/ §22.7.2（IMPL 3 段階）/ §22.8.0 / §22.8.3
- `docs/specs/20260513_shogi_save_ux_design.md`（SAVE-UX 中核原則 §2.3 modal/alert 不使用）
- `docs/specs/20260513_shogi_save_ux_warn_aggregation_design.md`（aggregation allow-list、本 closure 時点で `storage-corrupted` 追加なし）
- PR [#88](https://github.com/kazuo1970takahashi-sketch/shogi/pull/88) / [#89](https://github.com/kazuo1970takahashi-sketch/shogi/pull/89) / [#90](https://github.com/kazuo1970takahashi-sketch/shogi/pull/90) / [#91](https://github.com/kazuo1970takahashi-sketch/shogi/pull/91) / [#92](https://github.com/kazuo1970takahashi-sketch/shogi/pull/92) / [#93](https://github.com/kazuo1970takahashi-sketch/shogi/pull/93) / [#94](https://github.com/kazuo1970takahashi-sketch/shogi/pull/94)

## 24. SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT-DESIGN-CHECK（v2.4 追補 / boot timing 論点の解消と IMPL-LIGHT 最小スコープ確定）

- 種別: **docs-only design check**（実装はしない / §11〜§23 を改訂しない）
- 対象 main HEAD: `b39eb40`（PR #95 closure refinement squash merge 後）
- Task ID: `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT-DESIGN-CHECK`
- 目的: §22.7.0 で論点として登録した「boot timing と indicator UI 初期化の衝突可能性」を `shogi_v4.html` の現状コードを直視して解消し、`SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT` の **最小実装スコープを確定** する
- 起源: §23.5 第一推奨の前提条件「§22.7.0 boot timing 論点を事前に解く」/ §22.7.0 IMPL-LIGHT 着手時の検討選択肢 4 案

### 24.1 本 design check の目的と非目的

#### 24.1.1 目的

1. `load()` / `initApp()` / `notifySaveWarning()` / `showMsg()` / `updateSaveWarningIndicator()` を HEAD `b39eb40` で読み取り、boot 時の DOM 状態を確定する
2. §22.7.0 の boot timing 懸念が **実態として成立するか** を検証する
3. 実装方式候補 A〜D（§22.7.0 既出）を比較し、IMPL-LIGHT で採用すべき方式を 1 つに絞る
4. `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT` 最小実装スコープを Task ID / 範囲 / metadata / message 起点で確定する
5. 司令塔判断 1 回で次の impl PR を起動できる状態にする

#### 24.1.2 非目的（本 PR ではやらない）

- 実装変更（`shogi_v4.html` / test / docs/specs / CI 設定）
- 仕様確定の最終形（最終 message は impl 着手時に運用者レビューを挟む）
- §22.7.0 の他 3 案（候補 B / C / D）の実装可能性検証（本 PR は採用方式 1 つに絞る、棄却理由を明記するのみ）
- PARSE-LOAD-001 / 002 / 002A / 002B / FULL / STORAGE-CORRUPTED-HANDLING-IMPL の検討（§22.7.2 で 3 段階整理済、本 PR は LIGHT のみ）

### 24.2 `load()` / `initApp()` / `notifySaveWarning()` の関係（実装読み取り結果）

#### 24.2.1 boot 順序（HEAD `b39eb40` で確認）

```
[index.html parse]
  ├ <span id="save-warning-indicator" class="save-warn-pill" hidden></span>   ← shogi_v4.html:105
  ├ <div id="pane-reg">
  │   └ <div id="reg-msg"></div>                                              ← shogi_v4.html:130
  └ <script> ... </script>                                                    ← 関数定義
    ├ function load(){ ... }                                                  ← shogi_v4.html:443-459
    ├ function showMsg(text, type){ getElementById('reg-msg'); ... }          ← shogi_v4.html:2987-2990
    ├ var saveWarningIndicatorState={count:0};                                ← shogi_v4.html:3007
    ├ function recordSaveWarningForIndicator(options){ try { ... } catch }    ← shogi_v4.html:3035-3044
    ├ function updateSaveWarningIndicator(){ getElementById; if(!el)return; ← shogi_v4.html:3050-3067
    ├ function notifySaveWarning(options){ showMsg + console.warn + indicator } ← shogi_v4.html:3084-3140
    ├ function initApp(){ load(); ... }                                       ← shogi_v4.html:5791-5804
    └ document.addEventListener('DOMContentLoaded', function(){
        bindHeaderEvents(); bindRegistrationEvents(); bindTabEvents();
        bindReportEvents(); initApp();   ← initApp が呼ばれるのはここ        ← shogi_v4.html:5806-5812
      });
```

つまり実行順序は:

1. ブラウザが `<script>` 末尾まで HTML パース → `<span id="save-warning-indicator">` / `<div id="reg-msg">` は **DOM 上に既に存在**（display 状態に関わらず `getElementById` で取得可能）
2. JS が `notifySaveWarning` / `updateSaveWarningIndicator` 等の **関数定義を完了**
3. `saveWarningIndicatorState={count:0}` も初期化完了
4. `DOMContentLoaded` イベント発火 → callback 内で `bindHeaderEvents` → `bindRegistrationEvents` → `bindTabEvents` → `bindReportEvents` → `initApp()`
5. `initApp()` 内の最初の処理が `load()`（5792 行）

**この時点で DOM・関数・state すべて完全に初期化済み**。

#### 24.2.2 `notifySaveWarning` 経路の安全性確認

| 関数 | 行 | DOM 依存 | 失敗時挙動 |
|---|---:|---|---|
| `notifySaveWarning` | 3084 | 間接（`showMsg` / `updateSaveWarningIndicator` 経由）| **例外を投げない契約**（line 3082-3083 に明記）|
| `showMsg` | 2987 | `getElementById('reg-msg')` | `reg-msg` は **静的 HTML（line 130）** で常に存在。null 返却の可能性なし |
| `recordSaveWarningForIndicator` | 3035 | 間接 | **try/catch で保護**（line 3036-3043）、例外時は `console.warn` のみ |
| `updateSaveWarningIndicator` | 3050 | `getElementById('save-warning-indicator')` | 静的 HTML（line 105）で常に存在。さらに **`if(!el)return;` で null-safe**（line 3052-3053）/ try/catch で保護 |

3 層の safety net:
1. 静的 HTML 要素は DOMContentLoaded 時点で必ず存在
2. helper 関数は `if(!el)return;` の null check で防御
3. `recordSaveWarningForIndicator` / `updateSaveWarningIndicator` は try/catch で例外を握り潰す

### 24.3 boot timing / indicator UI 初期化の確認結果

#### 24.3.1 §22.7.0 懸念の現実評価

§22.7.0 では「boot timing で indicator UI がまだ DOM に存在せず、警告が表示されないリスク」を論点化した。HEAD `b39eb40` での実装直視結果:

| 軸 | §22.7.0 仮定 | 実装直視結果 |
|---|---|---|
| `load()` 呼出時の DOM 状態 | 未確認（仮定リスクあり）| ✅ **DOMContentLoaded 内** で `initApp` → `load` が呼ばれるため、**全静的 HTML はパース済**（`reg-msg` / `save-warning-indicator` ともに line 105 / 130 に静的存在）|
| `notifySaveWarning` の副作用 | 「indicator UI 未構築なら silent skip の可能性あり」 | ✅ **DOM は存在し** showMsg は writable、`updateSaveWarningIndicator` も null-safe / try/catch 保護で安全 |
| 例外リスク | 未確認 | ✅ `notifySaveWarning` 契約として「例外を投げない」と line 3082-3083 に明記。テスト経路でも保護される |

#### 24.3.2 結論

§22.7.0 の懸念は **理論上の安全策として有効**だが、**`shogi_v4.html` 現状の構造（HEAD `b39eb40`）では実態として顕在化しない**。理由:

1. `load()` は `DOMContentLoaded` 経由で呼ばれる → DOM パース完了済
2. 必要な DOM 要素（`reg-msg` / `save-warning-indicator`）は静的 HTML として常に存在
3. helper 関数群は null-safe + try/catch で多層防御済
4. 仮に boot 順序を将来変更しても、上記 3 層 safety net で silent skip するだけで例外は出ない（contract preserved）

→ **IMPL-LIGHT で候補 A（load() 内で notifySaveWarning を直接呼ぶ）を採用しても安全**。

#### 24.3.3 クラッシュ防止と「表示保証」の区別（cowork 指摘への対応）

§24.2.2 / §24.3.2 の 3 層 safety net は **クラッシュ防止の保証** であり、**実際に警告が表示される保証**とは別である。両者を混同しないため、本節で「表示経路の成立条件」を追加検証する。

`notifySaveWarning` の表示経路は以下 4 要素に依存:

| 依存要素 | 種別 | 初期化位置（HEAD `b39eb40`）| `load()` 実行時点で利用可能か |
|---|---|---|---|
| `<div id="reg-msg">` | 静的 DOM | [shogi_v4.html:130](shogi_v4.html:130)（HTML パース完了で存在）| ✅ DOMContentLoaded 内で実行されるため必ず存在 |
| `<span id="save-warning-indicator">` | 静的 DOM | [shogi_v4.html:105](shogi_v4.html:105)（同上）| ✅ 同上 |
| `saveWarningIndicatorState={count:0}` | module 変数 | [shogi_v4.html:3007](shogi_v4.html:3007) module top-level `var` | ✅ script 実行時に即初期化 |
| `SAVE_WARN_AGGREGATION_WINDOW_MS` / `SAVE_WARN_AGGREGATED_MESSAGE` / `SAVE_WARN_AGGREGATABLE_KINDS` / `saveWarningAggregationState={lastTimestampByKey:{}}` | module 定数 / 変数 | [shogi_v4.html:3015-3023](shogi_v4.html:3015) module top-level | ✅ script 実行時に即初期化 |

→ `notifySaveWarning` / `recordSaveWarningForIndicator` / `updateSaveWarningIndicator` は **`initApp()` 内で初期化される必須 state に依存していない**。すべて module top-level の `var` / 静的 DOM のみを参照する。

**表示保証の結論**: `load()` 呼出時点で `notifySaveWarning` の **表示経路（showMsg `innerHTML` 書き換え + indicator count 更新 + console.warn）はすべて成立可能**。「3 層 safety net = クラッシュしない」だけでなく「依存 state が module top-level で初期化済 = 表示経路が成立する」ことが物的に確認できる。

ただし IMPL-LIGHT 実装時の **再確認義務**:
- 仮に将来 `notifySaveWarning` 経路に「`initApp()` 後初期化の state」（例: bind 後の event handler ハッシュ等）への依存が追加された場合、本 §24.3.3 の前提は崩れる
- そのため IMPL-LIGHT 着手時に同じ依存検証を 1 回実施することを **起票判断材料 §24.8.3 に追加**
- 「表示される」と断定はせず「表示経路が成立する」「実態として顕在化しにくい」表現に留める

### 24.4 実装方式候補 A〜D の比較

§22.7.0 で挙げた 4 候補を HEAD `b39eb40` 読み取り結果に基づいて評価:

| 候補 | 概要 | 利点 | 不利点 | 評価 |
|---|---|---|---|---|
| **A** | `load()` 内で `notifySaveWarning` 直接呼出 | 最小変更（+5〜10 行）/ PARSE-MASTER-003 と並列構造 / 既存 4 系統設計の自然な拡張 / 既存 safety net が機能する | 理論的には DOM 未初期化リスクだが §24.3 で実態無効と確認済 | ✅ **採用** |
| B | boot 専用一時 pending queue + UI 初期化後に flush | boot 順序が将来変わっても安全 / 抽象化が綺麗 | 新規 queue 機構の導入で実装量が増える / 既存 4 系統に存在しないパターン | ❌ 棄却（候補 A で十分、過剰設計）|
| C | `load()` ではフラグだけ立て、`initApp()` のレジ画面初期化後に通知 | 責務分離が明確 / DOM 確実 | `load()` と `initApp()` で責務がまたがる / 状態渡しのため module-scope 変数追加 | ❌ 棄却（責務分散の方が読みにくい、候補 A で十分）|
| D | `notifySaveWarning` 自体を boot-safe に大改修 | 4 系統統一の一般解 / 将来の boot 時 warn にも効く | 影響範囲が広い（既存 21 callsite すべてに影響評価が必要）/ IMPL-LIGHT スコープを大幅超過 | ❌ 棄却（IMPL-LIGHT には重すぎる、別タスク `SAVE-UX-NOTIFY-BOOT-SAFE` として将来検討）|

### 24.5 推奨方式: 候補 A（`load()` 内で `notifySaveWarning` 直接呼出）

#### 24.5.1 採用根拠

- 最小実装（§22.7.0 から候補 A の不利点が実態無効と確認、§24.3）
- PARSE-MASTER-003 (`syncBranchMasterOnSave`) と **設計対称性** を保つ（既存 4 系統設計と整合）
- 既存テスト基盤（grep ベース structural assert + test_branch_master.js style）と整合
- 大会運営中の UX を変えない（boot 時のみの追加発火、observed 時のみ表示）
- §16〜§22 の設計判断（kind: `storage-corrupted` / aggregateKey: `storage-corrupted:state` / aggregation 対象外）を踏襲

#### 24.5.2 採用しないが原則は守る項目

- 候補 D の `notifySaveWarning` boot-safe 改修は **本 IMPL-LIGHT では実装しない** が、将来 boot 時 warn が増えた場合に再評価する論点として残す（仮 Task ID: `SAVE-UX-NOTIFY-BOOT-SAFE`、§24.7 参照）
- IMPL-LIGHT は「最小成功」のためのもので、4 系統横断の根本対応は別タスクとして切り出す

#### 24.5.3 PARSE-MASTER-003 との関係: 「metadata 並列 / timing 非対称」（cowork 指摘への対応）

§24.5 / §24.6 で「PARSE-MASTER-003 と並列構造」と書いた表現は **metadata レベルの並列** を指す。**呼出 timing は決定的に非対称** であることを明示する。「並列構造だから aggregation 挙動も同じ」と誤読されないようにする。

**metadata は並列**:

| metadata 軸 | PARSE-MASTER-003（既存）| PARSE-LOAD-003（IMPL-LIGHT 候補）|
|---|---|---|
| `kind` | `'storage-corrupted'` | `'storage-corrupted'`（同一）|
| `aggregateKey` | `'storage-corrupted:branch-master'` | `'storage-corrupted:state'`（`:state` で並列）|
| `severity` | `'warn'` | `'warn'`（同一）|
| `callsiteId` | `'PARSE-MASTER-003'` | `'PARSE-LOAD-003'`（inventory ID 流用、命名規則は §13 / §16.14）|
| aggregation 対象 | 対象外（`SAVE_WARN_AGGREGATABLE_KINDS` に未追加）| 対象外（同上）|

**timing は非対称**:

| timing 軸 | PARSE-MASTER-003（既存）| PARSE-LOAD-003（IMPL-LIGHT 候補）|
|---|---|---|
| 発火関数 | `syncBranchMasterOnSave()` ([shogi_v4.html:5286](shogi_v4.html:5286))| `load()` ([shogi_v4.html:443](shogi_v4.html:443))|
| 呼出経路 | `saveData()` 経由（user 操作 = 「大会データをコピー」ボタン押下時）| `initApp()` 経由（boot 時）|
| 発火頻度（最大）| **保存のたび**に発火しうる（user 操作の数だけ）| **セッションごと最大 1 回**（boot 時 1 回のみ）|
| UI 初期化状態 | 初期化済み（user 操作の時点で UI 構築完了）| `DOMContentLoaded` 内ではあるが、initApp() の最初の処理 |

**aggregation 方針（誤読防止）**:
- 初期実装（IMPL-LIGHT）では **両者とも aggregation 対象外** を維持（`SAVE_WARN_AGGREGATABLE_KINDS` 不変、§16.7）
- PARSE-LOAD-003 はセッションあたり最大 1 回発火なので、aggregation 対象外でも **頻発しない**
- PARSE-MASTER-003 は保存のたびに発火しうるが、運用フィードバックで実際の頻度を観察してきた
- **両者の aggregation 必要性は別軸で判断する**:
  - PARSE-LOAD-003: 1 回しか出ないため aggregation 不要（後続 PR でも見送り想定）
  - PARSE-MASTER-003: 既に運用観察中、フィードバック次第で aggregation 対象化検討
- 「metadata 並列 = aggregation 挙動も並列」ではない。**aggregation 判断は callsite ごとに独立**

**運用フィードバック次第での拡張余地（IMPL-LIGHT スコープ外、参考）**:
- PARSE-LOAD-003 の発火頻度が想定外に高い場合（マルチタブで頻繁に boot される運用等）→ aggregation 対象化 / 表示頻度制御を後続検討
- ただし本 IMPL-LIGHT では「**毎セッション 1 回表示は許容**」を初期方針として進める

### 24.6 次実装 PR `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT` 最小スコープ

#### 24.6.1 対象 callsite

- **PARSE-LOAD-003 のみ**（[shogi_v4.html:458](shogi_v4.html:458) 付近、`state=normalizeState(state);` の **直前** に warn 発火を挿入）
- 「初回起動（データなし、正常 case）」と「破損起源（PARSE-LOAD-001 / 002 が catch を通った後）」を **区別する**ため、`load()` 関数内に **`anyFailure` フラグ**（仮称）を導入:
  - PARSE-LOAD-001 catch（line 448）で `anyFailure=true;`
  - PARSE-LOAD-002 catch（line 454）で `anyFailure=true;`
  - ループ完走後の PARSE-LOAD-003 直前で `if(anyFailure) notifySaveWarning({...});`
- これにより:
  - 初回起動（全 key が `!d` で continue、anyFailure false）→ warn 発火なし ✅ user 体験を変えない
  - 全 key 破損 → anyFailure true → warn 発火 ✅ 致命点で通知
  - v4 失敗 → v3 成功（PARSE-LOAD-002A）→ ループ内 `return;` で PARSE-LOAD-003 に到達しないため warn 発火なし ✅ 002A は IMPL-LIGHT 対象外（§24.7）

##### 24.6.1.1 判定境界条件（擬似コード、docs 上の論証）

「初回起動 vs 破損起源の全キー失敗」の境界を明示するため、docs 上の擬似コードを示す（IMPL では `anyFailure` 単独で同等の挙動を達成可能。`anySeenValue` は本擬似コードでは境界の可視化用に併記するが、impl では省略可）:

```text
anyFailure   = false   # getItem 例外 or JSON.parse 失敗が 1 回以上発生したか
anySeenValue = false   # 非 null 値を 1 回以上読んだか（境界可視化用、impl では省略可）

for key in ['shogi_v4', 'shogi_v3']:    # STORAGE_KEY + LEGACY_STORAGE_KEYS
  try:
    raw = localStorage.getItem(key)
  catch:
    # PARSE-LOAD-001: SecurityError 等
    anyFailure = true
    continue

  if raw == null:
    # キー未保存 / クリア後 / 初回起動の正常状態
    # → anyFailure に含めない
    continue
  ...
  try:
    ...
    return                    # 成功 → ループ脱出（PARSE-LOAD-003 到達せず）
  catch:
    # PARSE-LOAD-002: JSON.parse 失敗（破損起源）
    anyFailure = true
    continue

# ─ ループ完走（どのキーでも state 復元できなかった）─

# 初回起動 / 全キー未保存（getItem 成功 + 全 null）:
#   anySeenValue == false && anyFailure == false
#   → warn しない（regression 防止: 新規 user が起動時に warn を見ない）

# 破損起源の全キー失敗（getItem 例外 or JSON.parse 失敗を経由）:
#   anyFailure == true
#   → PARSE-LOAD-003 warn 候補（notifySaveWarning 発火）

# 中間状態（getItem 成功 + 値あり + ループ内 return しない）は構造上発生しない
#   （成功時は必ず return、失敗時は anyFailure=true）

if anyFailure:
  notifySaveWarning({...})    # PARSE-LOAD-003

state = normalizeState(state)  # 初期 state へ rollover
```

判定境界の整理:

| 状況 | `anyFailure` | `anySeenValue` | warn 発火 | 備考 |
|---|---|---|---|---|
| 初回起動（全キー null）| false | false | ❌ | regression 防止対象 |
| 全キー getItem 例外（LOAD-001 経由）| true | false | ✅ | 破損起源 |
| 全キー JSON.parse 失敗（LOAD-002 経由）| true | true | ✅ | 破損起源 |
| 一部 getItem 例外 + 一部 null | true | false | ✅ | 破損起源（部分的 failure 検知）|
| v4 fail → v3 success（PARSE-LOAD-002A）| - | - | ❌ | ループ内 return で PARSE-LOAD-003 不到達、IMPL-LIGHT 対象外（§24.7）|
| v4 null → v3 success | - | - | ❌ | 同上、正常 fallback |

**重要な意図**:
- `anyFailure==false && anySeenValue==false`（初回起動）は **明示的に warn を出さない**: 新規 user の初回起動時に「保存データの読み込みに失敗」warn が出るのは regression
- `anyFailure==true` は「破損が 1 回以上観測されたが state 復元できなかった」を意味し、PARSE-LOAD-003 の本来意図と一致
- 本擬似コードは docs 上の論証であり、impl では `anyFailure` 単独で十分（`anySeenValue` は説明補助）。impl 時に変数名・コメントは確定する

#### 24.6.2 metadata（§22.5.1 / §16.13 と整合）

```js
notifySaveWarning({
  message: '保存データの読み込みに失敗したため初期状態で起動しました（過去大会データの取込で復元できる場合があります）。',
...
- `kind: 'storage-corrupted'` ✅ 既存 4 系統 4 番目を継続使用
- `aggregateKey: 'storage-corrupted:state'` ✅ §16.5 で予約済、`:branch-master` と並列
- `severity: 'warn'` ✅ 初期案、error 昇格は §22.5.3 の判断通り後続検討
- `callsiteId: 'PARSE-LOAD-003'` ✅ §16 inventory ID 流用（PARSE-MASTER-003 と同じパターン、§16.14）
- aggregation 対象外維持（`SAVE_WARN_AGGREGATABLE_KINDS` 不変、§16.7）

#### 24.6.3 message 文言

- 起点案: §22.5.2 の 2 番目「保存データの読み込みに失敗したため初期状態で起動しました（過去大会データの取込で復元できる場合があります）。」
- 構造（§18.4.1 / §18.5.1 設計方針と整合）:
  - 症状: 「保存データの読み込みに失敗した」
  - 影響: 「初期状態で起動しました」
  - 次の行動（既存 UI 導線への参照）: 「過去大会データの取込で復元できる場合があります」
- 「初期化されました」「データが消えました」等の **断定表現は避ける**（user パニック防止）
- 最終文言は impl 着手時に運用者レビューで確定（PR #89 §18 → PR #90 §18.5.1 と同じ手順）

#### 24.6.4 test 追加

- 最小 1 セクション: `test/test_branch_master.js` または `test/test_a5_1_save_001.js` 系の structural assert pattern を流用
- 確認内容:
  - `shogi_v4.html` 内に `callsiteId:'PARSE-LOAD-003'` が出現する
  - 同 callsite 近傍に `kind:'storage-corrupted'` / `aggregateKey:'storage-corrupted:state'` / `severity:'warn'` が存在
  - message に主要語句が含まれる（「保存データ」「読み込み」「失敗」「初期状態」「取込」「復元」程度、§16 4 系統合計 21 → 22 callsite への sanity check 更新）

#### 24.6.5 想定差分規模

| ファイル | 増分 | 種別 |
|---|---|---|
| `shogi_v4.html` | +5〜10 行（`anyFailure` フラグ + notifySaveWarning 呼出）| 実装 |
| `test/` | +1 section（主要語句 assert）| テスト |
| `docs/notes/20260513_shogi_save_ux_status_map.md` | +5〜15 行（§16.13 4 系統合計 21 → 22 への update、または新 §25 で impl 完了状況）| docs |
| `HANDOFF.md` | +1 行（impl 完了ポインタ）| docs |

### 24.7 対象外（IMPL-LIGHT スコープに含めないもの）

| 項目 | 理由 |
|---|---|
| PARSE-LOAD-001 直接 warn 化 | LOAD-001 単独は LOAD-003 経由で warn が出るため二重通知になる、§24.6.1 フラグ方式で十分 |
| PARSE-LOAD-002 直接 warn 化 | 同上 |
| PARSE-LOAD-002A（v4 失敗 → v3 成功）対応 | ループ内 `return;` で PARSE-LOAD-003 に到達しないため、IMPL-LIGHT のフラグ方式では発火しない。これは意図的（§22.3.3 で「shogi_v3 deprecate で自然解消する可能性」既出）|
| PARSE-LOAD-002B 単独対応 | LOAD-003 と合流するため LIGHT で同時カバー |
| `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-FULL` | §22.7.2 第三案、現時点では重い |
| `SAVE-UX-STORAGE-CORRUPTED-HANDLING-IMPL` | §16.9 案 B、LOAD-002 を直接 warn 化する範囲、LIGHT の上位案として後続 |
| `notifySaveWarning` の boot-safe 大改修 | 候補 D 棄却（§24.4）、別タスク `SAVE-UX-NOTIFY-BOOT-SAFE` 仮称として将来検討 |
| severity `error` 昇格 | §22.5.3 で「IMPL-LIGHT では warn 案、error 昇格は後続判断」既出 |
| `alert` / `modal` での通知 | SAVE-UX-DESIGN §2.3 / §18.4.1 設計方針と矛盾 |
| 自動復旧 / バックアップ復元 | §17.12 / §18.10 「やらない候補」 |
| 外部送信 / ログ送信 | §19.5（PR #95 で §18.8.1 参照追加済）|
| schema_version bump 対応 | §17.9 弱点 #10 と本 audit `FLAG-COVERAGE`（§21.10.2）の責務 |

### 24.8 司令塔向け結論

#### 24.8.1 IMPL-LIGHT に進んでよいか

- ✅ **進んでよい**
- §22.7.0 の boot timing 懸念は本 design check で **実態無効** と確認（§24.3）
- 採用方式: **候補 A**（`load()` 内 `anyFailure` フラグ + `notifySaveWarning` 直接呼出）
- 4 系統 21 → 22 callsite への自然な拡張（aggregation 対象外維持、kind / aggregateKey 既存設計に整合）

#### 24.8.2 進む場合の Task ID

- `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT`（§22.7.2 第一案 / §23.5.1 第一推奨と同一）

#### 24.8.3 実装前に追加で確認すべきこと

| 項目 | 着手前に解くべきか | 解き方 |
|---|---|---|
| 最終 message 文言 | 推奨 ✅ | impl PR 起票時に運用者レビュー（§24.6.3 案を起点）|
| `anyFailure` フラグ方式の妥当性 | impl 着手で十分 | コード読みやすさを優先（変数名・コメントは impl PR で確定）|
| test の置き場所（test_branch_master.js 流用 or 新規）| impl 着手で十分 | grep structural assert pattern が確立済（§16.13 / §21 で再利用）|
| `SAVE_WARN_AGGREGATABLE_KINDS` の sanity check | impl 着手で十分 | 4 系統合計 21 → 22 に更新、aggregation 対象外なので allow-list は変更しない |
| 既存 test の sanity 値 21 を 22 へ更新 | impl 着手で十分 | `__EXPAND_BLOCKS.length` の sum 比較を更新（§16.13 PR #87 と同パターン）|

→ **追加の design check は不要**。impl PR 起票時に最終 message のみ運用者レビューを挟めば即着手可能。

#### 24.8.4 IMPL-LIGHT 後の次フェーズ

- LIGHT 着地 → 観察期間 → `SAVE-UX-STORAGE-CORRUPTED-HANDLING-IMPL`（§16.9 案 B、LOAD-002 まで拡張）または `SAVE-UX-STATE-RESTORE-HANDLING-IMPL-FULL`（LOAD-001 含む全件）の起票判断
- 観察期間中の指標: 「reload 後に大会データが消えた」「古いデータで起動した」事案の有無
- 観察期間と並走して `SAVE-UX-ADDPLAYER-CORRUPTION-GUARD`（§21.10.2、支部マスタ側の中リスク後続化）を起票判断するのも可（独立に進められる）

### 24.9 関連 docs / コード

- `docs/notes/20260513_shogi_save_ux_status_map.md`
  - §13（callsiteId 命名規則）/ §16（parse handling inventory）/ §16.5 / §16.6 / §16.7 / §16.13 / §16.14
  - §17〜§21（支部マスタ破損対応、参照系）
  - §22（state restore inventory）/ §22.5.1 / §22.5.2 / §22.5.3 / §22.7.0 / §22.7.2 / §22.8 / §22.3.3
  - §23（closure refinement）/ §23.5 第一推奨（本 design check の起源）
- `docs/specs/20260513_shogi_save_ux_design.md`（SAVE-UX-DESIGN §2.3 modal/alert 不使用）
- `docs/specs/20260513_shogi_save_ux_warn_aggregation_design.md`（aggregation allow-list、本 IMPL-LIGHT 着手時にも変更なし）
- 主要コード参照（HEAD `b39eb40`、すべて読み取りのみ、本 PR では変更しない）:
  - [shogi_v4.html:105](shogi_v4.html:105) `<span id="save-warning-indicator">` 静的 HTML
  - [shogi_v4.html:130](shogi_v4.html:130) `<div id="reg-msg">` 静的 HTML
  - [shogi_v4.html:443-459](shogi_v4.html:443) `load()` 関数本体（IMPL-LIGHT 実装対象）
  - [shogi_v4.html:448](shogi_v4.html:448) PARSE-LOAD-001（`anyFailure=true` 挿入候補）
  - [shogi_v4.html:451](shogi_v4.html:451) PARSE-LOAD-002（同上）
  - [shogi_v4.html:458](shogi_v4.html:458) PARSE-LOAD-003（notifySaveWarning 挿入候補、直前）
  - [shogi_v4.html:2987-2990](shogi_v4.html:2987) `showMsg()`（DOM 取得経路）
  - [shogi_v4.html:3035-3044](shogi_v4.html:3035) `recordSaveWarningForIndicator()` try/catch 保護
  - [shogi_v4.html:3050-3067](shogi_v4.html:3050) `updateSaveWarningIndicator()` null-safe + try/catch
  - [shogi_v4.html:3084-3140](shogi_v4.html:3084) `notifySaveWarning()` 例外を投げない契約
