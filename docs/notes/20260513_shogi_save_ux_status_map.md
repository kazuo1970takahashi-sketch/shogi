...
2. **既存 UI を前提に書く**: 「マスタタブで再読込・初期化・取込で復旧できます」など、既存 button label と一致する語彙を使う
...
```
支部マスタが破損しているため自動同期をスキップしました（大会データのコピーは継続）。マスタタブから取込・初期化で復旧できます。
```

- 末尾に「マスタタブから取込・初期化で復旧できます」を追加（10 文字程度）
- 既存 UI button label `📂 マスタをインポート` / `📛 マスタをリセット` と語彙整合
...
**暫定推奨**: **L-1**（取込・初期化への参照で行動性が最も高い）。impl 着手前に運用者レビューで確定。
