...
- companion test（main の `test/test_pr_gate_scripts.sh`）の orphan base への移植は **完了（AUTO001-GATE-TEST-PORT-001）**: `test/test_pr_gate_scripts.sh` を進化後 orphan scripts 向けに *適応移植*（AS-IS ではない）。`orphan-dev` profile / head-CAS（`--match-head-commit`）/ `--expect-head` 不一致停止 / Step7/8 の `git ls-remote` を PATH stub 化（実 origin・network 非依存）/ gh api 非依存 / dry-run 既定 / `--delete-branch`・`--auto` 不使用 を mock で固定。`run_tests.sh` に guarded block で統合（npm test で回帰保護）。**runbook（main 系 `docs/operations/shogi_tour_auto_pr_gate_runbook.md`）は移植せず参照資産**（orphan-canonical な運用手順は本ドキュメントを正とする）。
