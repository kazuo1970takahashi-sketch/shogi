...
| **Ready Approved**（= Ready Recommended） | AI レビュー / ChatGPT 司令塔が「Ready 化してよい」と **判断・提案を出した** だけの状態（実行承認ではない） | 髙橋さん（明示許可判断）→ その後 Claude Code（Ready 化実行） | × | **必須**（実行は髙橋さん明示許可後の "Ready Execution Approved" 段階で初めて可） | 髙橋さん明示許可なしに Claude Code が自律 Ready 化することを禁止 |
...
| **Merge Approved**（= Merge Recommended） | AI レビュー / ChatGPT 司令塔が「merge してよい」と **判断・提案を出した** だけの状態（実行承認ではない） | 髙橋さん（明示許可判断）→ その後 Claude Code（squash merge 実行） | × | **必須**（実行は髙橋さん明示許可後の "Merge Execution Approved" 段階で初めて可） | 髙橋さん明示許可なしに Claude Code が自律 merge することを禁止 |
...
#### Ready Approved（= Ready Recommended）
- **意味**：AI レビュー（cowork / Codex）または ChatGPT 司令塔が「Ready 化してよい」と **判断・提案を出した状態**。
- **重要な注意**：これは「Claude Code が自律的に Ready 化してよい状態」**ではない**。実行承認ではなく、**推奨判断** に留まる。
- Claude Code が実際に Ready 化を実行するには、以下のいずれかが必要：
  - (a) 髙橋さんの明示許可
  - (b) 髙橋さんが明示的に認めた依頼文（例：「PR #xxx は A / Go、Ready 化のみ実行、merge 禁止」等）
- 上記 (a) (b) が揃った段階が実質的な "Ready Execution Approved"（実行承認段階）であり、ここで初めて Claude Code は Ready 化を実行できる。
- v0.1 では **AI 判断だけを根拠に Claude Code が自律 Ready 化することを禁止** する。

#### Ready Done
- PR が Ready for Review になった。
- 場合によっては最終レビュー（cowork / Codex）を再依頼。
- merge 可否判断が出たら Merge Approved（= Merge Recommended）へ。

#### Merge Approved（= Merge Recommended）
- **意味**：AI レビュー（cowork / Codex）または ChatGPT 司令塔が「merge してよい」と **判断・提案を出した状態**。
- **重要な注意**：これは「Claude Code が自律的に merge してよい状態」**ではない**。実行承認ではなく、**推奨判断** に留まる。
- Claude Code が実際に squash merge を実行するには、以下のいずれかが必要：
  - (a) 髙橋さんの明示許可
  - (b) 髙橋さんが明示的に認めた依頼文（例：「PR #xxx は Ready / OPEN、head SHA xxx、A / Go、squash merge のみ実行、branch 削除禁止」等。対象 PR 番号・禁止解除対象・実行範囲が明示されていること）
- 上記 (a) (b) が揃った段階が実質的な "Merge Execution Approved"（実行承認段階）であり、ここで初めて Claude Code は merge を実行できる。
- v0.1 では **AI 判断だけを根拠に Claude Code が自律 merge することを禁止** する。
...
Claude Code の完了報告 / PR 本文 / PR コメントに以下の欄を含める設計とする（v0.1 は設計のみ、実装は IMPL-LIGHT で行う）。**全 16 欄を等しい重さで「必須」とすると短いタスクで完了報告が肥大化し形骸化するため、Core 5 と Standard 11 の 2 レイヤに分ける。**

### 6.1 Core 5（省略不可）

以下の 5 欄は **すべての完了報告 / PR コメント / handoff に必ず含める**。省略不可。これが揃わない完了報告は不完全とみなす：
...
### 6.2 Standard 11（原則記載・短いタスクでは簡略化可）

以下の 11 欄は **原則記載**。ただし以下の扱い：

- **短いタスク（Level 0〜2、文言 1 行の IMPL-LIGHT 等）では簡略記載または省略可**
- 省略する場合でも、PR 本文・PR コメント・GitHub UI（branch / commit / file 一覧）等から追跡できる状態にする
- **Level 3 以上、または危険操作（Ready 化 / merge / snapshot 更新 / release / deploy / publish / branch 削除等）を含む場合は省略しない**
...
### 6.3 Core 5 と Standard 11 の違い（要約）

| レイヤ | 扱い | 省略可否 | 危険操作を含む場合 |
|---|---|---|---|
| **Core 5** | 全完了報告で必ず含める | **省略不可** | 当然必須 |
| **Standard 11** | 原則記載・短いタスクでは簡略化可 | 短いタスクで簡略 / 省略可 | **Level 3 以上は省略しない** |

判断基準：Standard 11 を省略すべきかどうか迷ったら、含める。形骸化を避けるための簡略化であって、可視性を下げるためではない。

### 6.4 記入例（本 PR を想定、全 16 欄記載）
...
### 6.5 短いタスクでの簡略記入例（Level 2 IMPL-LIGHT 文言 1 行修正を想定）

```
[Core 5]
- Next Action: cowork 向けレビュー依頼を PR コメントに投稿
- Next Owner: 髙橋さん（レビュアー渡し）
- Blocked By: Review
- Allowed Without Human Approval: Draft PR 維持・修正・PR コメント投稿
- Requires Human Approval: Ready 化 / merge

[Standard 11 簡略版]
- Task ID / PR / Branch / Commit / Status: GitHub UI 参照
- Risk Level: Level 2 IMPL-LIGHT
- Tests: 全 N stanza PASS / FAIL=0
- それ以外（Forbidden Actions / Changed Files / Review Needed / Handoff URL）: 標準値 + 変更ファイル diff 参照
```

Level 3 以上ではこの簡略化は禁止。Standard 11 を完全記載する。
...
**重要 — Short Prompt の使用条件（自律実行トリガーではない）**：

- **Short Prompt は、髙橋さんの明示許可後に使う「実行指示の伝達フォーマット」である**。
- Claude Code は、**Short Prompt の存在だけを根拠に**、以下の操作を自律実行してはいけない：
  - Ready 化（Mark as Ready for Review / Draft 解除）
  - merge / squash merge
  - VRT snapshot 更新
  - release / deploy / publish
  - branch 削除
  - その他 §8 人間承認必須操作すべて
- Short Prompt は「**許可後の伝達を短くするための形式**」であり、「**許可そのもの**」ではない。
- 言い換えると、Short Prompt が PR コメントやチャットに存在していても、それは ChatGPT 司令塔またはレビュアー AI が **推奨案（Recommended）として提示しただけ** の可能性がある。実行可否は別途、髙橋さんの明示許可（または髙橋さんが明示的に認めた依頼文の提示）が必要。
- **Ready 化 / merge の Short Prompt が有効になる条件**：
  1. 対象 PR 番号が明示されている
  2. 禁止解除対象（例：「Ready 化のみ可、merge 禁止」「squash merge のみ可、branch 削除禁止」）が明示されている
  3. 実行範囲（head SHA / Ready 前状態 / 確認事項）が明示されている
  4. **かつ** 髙橋さんが明示的にその Short Prompt を「実行してよい」と認めている
- 上記 4 条件が揃って初めて、Claude Code は Short Prompt に従って Ready 化または merge を実行できる（§5.2 の "Ready Execution Approved" / "Merge Execution Approved" 段階に相当）。

**Ready 化 Short Prompt 例**（髙橋さん明示許可後の実行指示として）：
```
...
**merge Short Prompt 例**（髙橋さん明示許可後の実行指示として）：
```
