...
★ 残り6件のうち2件（`2人を同時に…` / `再戦になる…`）には「選手」が残る。
用語の全面統一（option ラベル `同じ選手` と `test_pairing_classify_001.js:151` の pin を含む）は**別スライス**。
...
変異は `test/tools/chg_inline_error_881_mutants.js` が実行時に生成する（19本・repo には置かない）。
...
- `bash test/run_e2e.sh` → **10/10 スイート PASS**
  - `chg_modal_inline_error_881.e2e.js`（新設）**77/0**
