...
- 通知: `console.warn('[TAG] callsite verify failed', {meta})`
- 当日 UI: なし
- 想定: 影響が局所的で、当日中の対応が不要なケース
- 既存対応: A-5.1-SAVE-001 〜 003b-3 の callsite warn、MASTER-V2-LASTCLASS の S03 / S05 / S22 warn は **現状すべて Level 0**
...
- 既存対応: A-5.1-SAVE-001 〜 003b-3 の一部（`removePlayer` / `addPlayer` / `bulkEditNames` など）は **既に Level 1 相当**（warn 文言「ブラウザを閉じる前にバックアップしてください」）
...
| quota exceeded（`save()` 内 `notifyError`） | localStorage 容量超過 | 既存 `save()` の catch | Level 1〜2（alert あり） | Level 2 想定（再検討） | 現状は `notifyError` で alert。Level 5 に近いが緊急性が高く例外的に許容。SAVE-UX-MIN-NOTIFY ではスコープ外 |
...
9. **PR #59 / #60 / #61 の保存安全化と整合しているか**
   ...
   - MASTER-V2-LASTCLASS-DESIGN / IMPL（#60 / #61）の通知方針（`showMsg(.., 'warn')` + `console.warn`、alert / rollback / retry なし）と整合しているか
