...
`syncBranchMasterOnSave()` → `updateBranchMasterFromTournament()` → `saveBranchMaster(master)`
...
|---|---|---|---|
| S03 | `handlePastParticipantClassAdd` 内の **既登録者クラス変更分岐**（`changePlayerClass()` 成功 → `saveBranchMaster(master)` を呼ぶ経路） | `last_class` | 既登録者のクラス変更経路に限定。未登録の新規追加分岐（`addPlayerFromMaster` 側）は master の `last_class` を更新しないため対象外（誤 warn 防止）。詳細は §7.1 |
| S05 | `handleSuggestClassAdd` 内の **既登録者クラス変更分岐**（`changePlayerClass()` 成功 → `saveBranchMaster(master)` を呼ぶ経路） | `last_class` | 既登録者のクラス変更経路に限定。未登録の新規追加分岐は master を更新しないため対象外（誤 warn 防止）。詳細は §7.2 |
| S22 | 会員マスタ編集モーダル保存 | `last_class` / `member` / `grade` / `city` | 編集モーダルの保存ボタン押下経路。expected 値は `applyMasterMemberEdit` 適用後の in-memory master member 値から取得する。詳細は §7.3 |
...
| S30 | `syncBranchMasterOnSave` の 2 段保存 | 複数 member 一括の batch verify が必要。Must の単一 member verify とは別設計。暫定 Task ID 候補: `MASTER-V2-S30-BATCH-VERIFY` |
...
7. member 不在なら `false`
8. `member[field]` と `expected` を比較する（呼出側で `expected` は in-memory master member の正規化済み値から取得すること。詳細は §7.3）
...
S03 の verify 対象は `handlePastParticipantClassAdd` 関数全体ではなく、**既登録者のクラス変更分岐のみ** に限定する。

具体的には、`changePlayerClass()` が成功し、その直後に `saveBranchMaster(master)` が呼ばれた経路にだけ verify を入れる。

未登録の新規追加分岐（`addPlayerFromMaster` 側）では master の `last_class` を更新せず `saveBranchMaster(master)` も呼ばないため、ここで verify を入れると **false negative / 誤 warn になる**（保存していない値を「保存できなかった」と誤検知する）。よって、新規追加分岐では `verifyMasterFieldPersisted` を呼ばないこと。
...
S05 の verify 対象は `handleSuggestClassAdd` 関数全体ではなく、**既登録者のクラス変更分岐のみ** に限定する。

具体的には、`changePlayerClass()` が成功し、その直後に `saveBranchMaster(master)` が呼ばれた経路にだけ verify を入れる。

未登録の新規追加分岐では master の `last_class` を更新せず `saveBranchMaster(master)` も呼ばないため、ここで verify を入れると **false negative / 誤 warn になる**。よって、新規追加分岐では `verifyMasterFieldPersisted` を呼ばないこと。
...
verify の expected 値は **フォーム生値から直接作らない**。`applyMasterMemberEdit()` 適用後の **in-memory master member（= 保存直前 / 直後の `master.members[i]` の正規化済み値）** から取得する。

つまり verify 対象の expected は、保存直前または保存後の in-memory master member に保持されている正規化済み値とする。

特に `city` は `normalizeCity()` により trim / 最大 20 文字丸めが入るため、フォーム生値を expected にすると **false negative**（実際は正しく保存されているのに verify が `false` を返す）になる。`last_class` / `member` / `grade` も同じく、実際に master へ格納された値（= `applyMasterMemberEdit` が `target.member = options.member` などで更新した後の値）を expected として用いる。
...
`name` / `yomi` を本タスクで対象外とする理由:

- `name` は既存 `verifyMasterPersisted`（MASTER-001 / A-5.1 系の name 専用 verify）の対象であり、責務が重なるため、本タスクでは扱わない
- `yomi` は補完・入力経路が別系統（`updateBranchMasterFromTournament` 内の補完、サジェスト由来の補完など）であり、今回の `last_class` を含む編集モーダル保存 verify の最小対象から外す
- 本 PR では `last_class` / `member` / `grade` / `city` の 4 フィールドに限定する
...
7. S22：expected が `applyMasterMemberEdit` 適用後の master member 値から取得され、フォーム生値ではないこと（例: フォーム `city` に前後空白を含む値を入力 → `applyMasterMemberEdit` 経由で `normalizeCity` により trim 済の値が in-memory master に格納される → その trim 済値を expected として verify が `true` を返す。フォーム生値を expected にすると `false` になる）
8. S03 / S05：`changePlayerClass` を経由しない新規追加分岐では verify が呼ばれないこと（誤 warn 防止）
...
- S-Sh3：S30 `syncBranchMasterOnSave` の 2 段保存 batch verify（暫定 Task ID 候補: `MASTER-V2-S30-BATCH-VERIFY`）
