# Phase A-T Stage 2a 仕様書 v1.3:UI テストヘルパ実装(`expectClickable` + `clickAndExpectChange` + `triggerInputFileAndExpectChange`)

**配置先**: `docs/specs/20260506_0752_shogi_at_stage_2a_helpers_spec_v1.md`(同名上書き、v1.2 → v1.3)
**最終更新**: 2026-05-06 11:53 JST
**対象ブランチ**: `feat/phase-a-t-stage-2a-helpers`(main 1aa8e02 から派生、現 HEAD d6832af)
...
async function expectClickable(locator, options = {}) {
  // 段階 0: scrollIntoViewIfNeeded(v1.3 SF#1 で復活、A-T spec v1.3 §4.2 + DevSecOps v1.2.5 L4 整合)
...
| 20 | 読み込み実行(ファイル) | `#loadFile`(隠し file input、L94) | ファイルから state を復元 | `stateLoadedFromFile(filePath, expectedPlayersA, expectedPlayersB)` | state + 永続化 | **triggerInputFileAndExpectChange**(v1.3、Codex MF#2 反映) | **v1.3 反映:`#loadFile` 直接ターゲット、`setInputFiles` 直接実行で change イベント発火、`expectClickable` スキップ。`#load-pick-file` 仲介 button は使わない** |
...
test/
  helpers/
    ...
    getStateSnapshot.js                   # 新規(v1.3 で clickAndExpectChange.js から分離、両ヘルパ共通利用)
...
const shogiAssertions = {
  ...
  // #20 読み込み実行(ファイル) - v1.3 再設計(Codex MF#2 反映)
  ...
  stateLoadedFromFile: (expectedPlayersA, expectedPlayersB) => ({
    ...
    // beforeClick は不要(setInputFiles の副作用で change イベントが自動発火、loadData が同期実行)
    // 必要に応じて afterClick で非同期完了待ち(現実装は loadData 同期、不要)
    meta: {
      primaryAssertions: 1,
      primaryTypes: ['state', 'localStorage'],
      operation: 'stateLoadedFromFile',
      description: 'ファイルから state を復元(triggerInputFileAndExpectChange で setInputFiles)',
    },
  }),
...
test.describe('A-T Stage 2a sanity', () => {
  ...
  test('正常系: ファイル読込が triggerInputFileAndExpectChange で緑になる(v1.3、Codex MF#2 実態保証)', async ({ page }, testInfo) => {
    // v1.3 Codex MF#2 反映: stateLoadedFromFile の実態保証
    // #loadFile(隠し file input、L94)を直接ターゲット、setInputFiles 直接実行
    await page.goto('/shogi_v4.html');
    ...
    // 補助 assertion: ファイル名表示や UI 反映が起きること
    await expect(page.locator('#a-list .player-row')).toHaveCount(1);
  });
...
- ✅ **`stateLoadedFromFile` factory の `triggerInputFileAndExpectChange` 経由実行(`#loadFile` 直接ターゲット、`setInputFiles` 直接実行)を §5.2 サンプル e2e で正常系 1 件検証**(v1.3、Codex MF#2 実態保証)
...
- 本仕様書(v1.3)が ChatGPT メタレビュー A 以上(v1.3 簡易確認、A 判定維持狙い)
- 本仕様書(v1.3)が Codex 独立レビュー A 以上(v1.3 再レビュー、B+ → A 判定狙い)
- 実装後、Codex フェーズ境界レビュー(Stage 2a 完了時) A 以上
...
| **`triggerInputFileAndExpectChange` の `setInputFiles` 副作用 change handler が非同期**(v1.3 回帰懸念) | change handler が非同期完了する前に `getStateSnapshot` を取ると state が古い可能性 | factory の `afterClick` で非同期完了待ちを明記(`stateLoadedFromFile` の現実装は同期だが、将来の async 化を想定)、サンプル e2e で実 state 反映を補助 assertion で確認 |
...
`masterImported`(#17)について Stage 2a では §3.4 末尾「実装時確認可」に注記して実装担当のテスト側責任とする。Stage 2b 仕様書では以下を正式化:
