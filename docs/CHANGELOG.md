...
- テスト: `test/e2e/pairing_card_compact_942.e2e.js`（実 Chromium・375px・30名2クラス）。
...
- テスト: `test/test_score_grid_fold_944.js`。#661 の受入テストと同じ型で、
  **構造を読むだけでなく純関数として実行**する（`vm` サンドボックスで `isSP` を振る）。
  ...
  変異すべてで赤を実測（details をやめる／モバイルでも open／window 幅の既定判定をやめる／
...
- **修正**: 「直す」で開いた対局は「終わった対局」の中の**その場**に、上段と同じ大きいカード（勝者ボタン・補助ラベル・理由ラベル・「変更」・同じ id）で出す。その場のカードで勝者を押し直してもその場に留まり、別の対局を操作したら従来どおり 1 行に畳まれる。上段で勝者を押した対局は従来どおり上段に残る（#943 の「直前に操作した 1 件」規則は不変）。「終わった対局（N）」の N にはその場のカードも数える。実装は「直前に操作した対局」の記憶をクラス別の値 1 つ `{owner, key, where}` にまとめ（key＝回戦＋顔ぶれは #943 のまま・where＝'top' | 'inplace'・owner＝`state.pairings[cls]` の配列そのもの＝再生成・回戦確定・リセット・復元のすべてで置き換わるので、同じ回戦・同じ顔ぶれが再び組まれても古い記憶は効かない・#950 の dsSlot と同じ型）、`buildCurrentPairingsHtml` でカードの行き先を分岐しただけ。判定・保存ロジック・bind は無改変。
- テスト: `test/e2e/finished_match_fold_943.e2e.js`（F4 系を意図的に更新: その場に開く／上段に出ない／件数／押し直しで留まる／別の対局で畳まれる。SNAP を上段・その場・下段の 3 つに分けた。再生成／復元で古い記憶が効かないことも固定）。`disclosure_state_950.e2e.js` は無改変で緑。
