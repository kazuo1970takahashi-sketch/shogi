...
| E | GitHub Pages 公開系のみを安全な別 branch に切り替え、main 履歴対応は別途判断 | **候補** | 外部公開面からの実データ除去を **先に** 実現できる。Risk Level を段階的に下げられる。 | repo 履歴上には残るため、Level 4 履歴対応の必要性は別途残る。Pages 設定変更が必要。 |
