...
- 明らかに架空と分かる命名規則を使う。**将来の完全架空 fixture では、自然人名・実地名に近い表現よりも、機械的・連番的に架空と分かる命名を優先する**。
  - **優先（推奨）**: `Fixture User 001` / `Synthetic Player 001` / `Dummy City 001` / `Fixture Branch 001` / `Fixture Tournament 001` のような、機械的・連番的・英語ベースの命名。連番により件数や順序が明示でき、実在人物・実在地名・実支部・実大会と誤認される余地を最小化できる。
  - **非推奨（後退案）**: `テスト 太郎`、`テスト県テスト市 1-2-3`、`テスト支部` のような、自然人名・実地名に近い「テスト〜」表現。直感的にはわかりやすいが、検索・スクリーンショット・第三者引用などで実在物と誤認される余地が残る。当面の fixture でやむを得ず使う場合に限る。
  - いずれの命名でも、実在する人物・住所・支部・地域・大会と一致しないことを fixture 設計時にレビューする。
- 実大会と誤認されない日付・地域・属性を使う（例: 過去日付なら明確に架空年度、未来日付なら明示的に架空、地域名は実在しない名称または `Dummy City 001` 形式を採用）。
...
#### 003G の前倒し条件（公開影響が強く疑われる場合）

- 標準では 003G は最後だが、**GitHub Pages 上で現行公開影響が強く疑われる場合**（例: 公開 URL からの実データアクセスが疑われる、外部からの問い合わせが発生、検索結果に該当する痕跡がある等）、**003G を 003H の前後で前倒ししてよい**。
- 前倒しのバリエーション例:
  - **003H 直前**: 撤去対象が公開済みかつアクセス可能であることを確認した上で撤去実行に進む。
  - **003H 直後**: 撤去後の公開反映ラグ・キャッシュ残存・外部派生コピーの状況を即時確認する。
  - **003F / 003D-4A と並行**: 公開影響評価を先回りで開始し、撤去ルート判断に反映する。
- 前倒しを採用する場合でも、以下の制約は変わらず適用する。
  - **本文取得・表示・保存は禁止**。HEAD / メタ情報（status code、ETag、Last-Modified、Content-Length 等）の確認に限定する。
  - スクリーンショット保存・外部キャッシュへの追加クエリ等、痕跡を増やす操作は行わない。
- **003G の前倒しは、現行公開からの撤去（003H）を遅らせる理由にしない**。前倒し評価が完了する前でも、明確な公開リスクがあれば撤去判断を独立に進めてよい。
