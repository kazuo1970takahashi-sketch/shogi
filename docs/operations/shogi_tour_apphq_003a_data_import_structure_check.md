...
| 4 | Git 履歴確認 | `git log --format=%H -- <path>`、`git log --name-only --format=%H -- <path>`、`git rev-list --count HEAD -- <path>` | SHA、対象パス、変更回数、追加/変更/削除の有無のみ |
...
`git log` は強力だが、commit message 本文には実値・固有値・個人情報・secret が混入している可能性を完全には排除できない。そのため、本手順書では git 履歴確認を以下のように安全側に制限する。

- 原則として、**SHA・対象パス・変更回数・追加/変更/削除の有無**を中心に確認する。
- `git log --oneline` のように **commit message を一覧表示する操作は避ける**。やむを得ず参照する場合でも、実値・固有値・個人情報・secret が含まれる可能性があるため、出力をそのままチャット・PR 本文・Runbook・コメント・AI に貼らない。
- 003B で AI へ報告する場合、**commit message 本文は原則出さない**。
- 報告は「履歴あり / 追加 commit あり / 変更回数 / 対象パス」などに限定する。
- 実データ疑いがある commit message は表示せず、人間ローカル確認へ回す。

参考までに、抽象表現での確認項目例：

- git 履歴の存在・件数確認
- 対象パスに関する追加 / 変更履歴の有無確認
- SHA のみ確認
...
- Git 追跡：
- top-level type：
- top-level keys：（キー名に個人情報・固有値・実データ疑いがある場合は伏せる。7.1 を参照）
- record count：
- key set：（キー名に個人情報・固有値・実データ疑いがある場合は伏せる。7.1 を参照）
- value type distribution：
- null / empty counts：
- risk：
- 判定：
...
### 7.1 top-level keys / key set 欄の取扱い

レポートの `top-level keys` および `key set` 欄は、**キー名自体に個人情報・固有値・実データ疑いがある場合は伏せる**。

- キー名が個人情報・固有値・実データを含む（あるいは強く示唆する）場合は、レポートに具体的なキー名を書かない。
- その場合は「**キー名に実データ疑いあり。値・キー名とも非表示**」と報告し、件数や型分布のみを扱う。
- キー名の安全性に迷う場合も**高リスク側に倒す**（伏せる側を選ぶ）。
- 既出の Section 3「基本原則」「キー名自体に個人情報が含まれる可能性がある場合は、キー名も伏せる」と同義であり、レポート欄でも同じ運用とする。

### 7.2 レポートに含めることを禁止する項目
