...
> 注（案 B の回帰範囲縮小リスク）: 案 B は短期安全性を優先する一方で、**旧 E2E 以外の安全な E2E も一時的に CI 対象外になる可能性がある**。003D-4E-1 では、これを「暫定的な回帰範囲縮小」として扱うのか、「safe project / safe directory に安全確認済み E2E を含める方式」として扱うのかを **明記する**。前者を選ぶ場合は、復元または safe 側への追加条件を後続タスク（003D-4E-1 完了後または別タスク）に明記する。
...
### 6.4 案 B / 案 C の第一実装選定基準

003D-4E-1 開始時の選定基準は、以下のバランスで判断する。

- **短期遮断の速さを最優先** するなら **案 B**（CI コマンド限定）。最短で CI 上の旧 E2E 実行リスクを止められる。
- **今後も安全確認済み E2E を safe 側に増やす運用** を重視するなら **案 C または safe project 方式** を優先する。safe 対象を継続的に拡張できる。
- 旧 E2E 以外にも残したい安全確認済み E2E がある場合は、案 B 単独だと §6.1 の回帰範囲縮小リスクを暫定許容することになるため、**案 C / safe project への移行を視野に入れる**。
- 判断軸は「短期遮断の速さ」と「安全な回帰範囲の維持」のバランス。
...
#### 12.2.1 旧 E2E が CI で実行されていないことの diff-safe 確認境界

- 旧 E2E が CI で実行されていないことの確認は、**CI ログまたは実行コマンド上で safe 側の対象のみが実行されていること** を確認する範囲に限定する。
- 確認時に **旧 E2E 本文、旧期待値リテラル、具体件数、具体クラス分布、固定日付・固定日時の具体値を表示・引用しない**。
- 確認の記録は「safe project / safe command が実行されたこと」「legacy / real-data dependent E2E が CI 対象外であること」を **抽象表現** で残す（具体パス名は最小限）。
- 旧 E2E の **本文や failure ログ・trace を確認に使わない**（仮に偶発的に出力された場合は、PR コメント・レビューコメント・artifact 共有のいずれにも転載しない）。
- PR 本文・コメント・workflow YAML / Playwright config の差分も同じ境界で扱う。
