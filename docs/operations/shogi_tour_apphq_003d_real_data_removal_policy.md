...
- GitHub Pages で対象 JSON に直接アクセス可能か（URL 構造のみ確認、本文アクセスはしない）
- GitHub Pages 公開影響があるか（影響評価は 003G）
...
4. **003G**：GitHub Pages 公開影響確認（撤去 PR の前後どちらかで実施、別 Runbook）
5. **003E**：Git 履歴対応要否判断（最後、別 Runbook + Level 4 承認）
...
- 現行 tree からの削除・置換は**別 PR・別承認**（標準 Approval Phrase 体系で対応可能）。
- **Git 履歴改変・force push は標準 Approval Phrase では解除不可**。専用 Runbook + 明示承認 + 影響評価が必要。
- **GitHub Pages 公開済み影響確認も別 Runbook と人間承認が必要**。
