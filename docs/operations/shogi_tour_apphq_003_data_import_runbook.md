...
- すべての氏名・支部名・属性値が、**明らかに架空と分かる命名規則**で構成されている。
- 実人物・実支部・実大会日付・実参加履歴と誤認されない名称・値であり、サンプル / fixture / dummy 等の前提が明示されている。
- 架空である根拠（確認日、確認者、確認方法、命名規則の説明）を docs に記録できる。
...
| 作業 | Risk Level | 根拠 |
|---|---|---|
| 本 Runbook の作成 | **Level 3 相当**（docs-only） | 実装変更はないが、実データ・個人情報候補（`data/import/20260412_participants.json`）の取扱い方針に関わるため、APP-OPS-003 の「docs-only でも個人情報 / Production / 権限に関わる場合は Level 3 以上」に従い、レビュー上は Level 3 相当として扱う |
...
- ただし、共有可能なのは、**実値を含まない運用方針・判定手順・リスク分類**に限る。
- **AI へ共有する前に**、Runbook 本文に実値・固有値・個人情報・secret が混入していないことを確認する。
- Runbook に、実データ由来の値、実名、支部名、段級位、参加者属性値、実ファイル内容を含めない。
- 対象 JSON の中身は AI に貼らない、添付しない、共有しない。
