...
3. `gh pr ready`（**Draft の場合のみ**。既に Ready ならスキップ。§12.4）
...
- PR が `OPEN`
- **`mergeable=MERGEABLE` かつ `mergeStateStatus=CLEAN`**（Should Fix 1）

> `mergeable=MERGEABLE` でも `mergeStateStatus` が `CLEAN` 以外（`BLOCKED` / `UNSTABLE` / `BEHIND` / `DIRTY` / 空 / `UNKNOWN`）の場合は `READY_CANDIDATE` にならず `NEEDS_REVIEW`（WARN）。出力には `mergeable` と `mergeStateStatus` の両方を明示する。
...
- `OPEN` かつ **`mergeable=MERGEABLE` かつ `mergeStateStatus=CLEAN`**（Should Fix 1。CLEAN 以外は `NEEDS_REVIEW`）
...
- **base が `production` の場合は NG（BLOCKED）**（Should Fix 2。profile 定義上 production 以外が条件）
...
| docs-only / test-only が base=production | profile 定義上 production 以外が条件（Should Fix 2） | `main-dev` / `production-minimal` 等、適切な profile を選ぶ |
...
- **`mergeable=MERGEABLE` だが `mergeStateStatus` が `CLEAN` 以外**（`BLOCKED` / `UNSTABLE` / `BEHIND` / `DIRTY` / 空 / `UNKNOWN`）: Should Fix 1。required check 未完・base 遅れ・保護ルール等。CI 完了や base 取り込みを待って再実行、または人間が確認。
...
`--execute` 中、Ready 化直後の再 gate が `READY_CANDIDATE` でなければ **merge せず停止**する（exit 10/20）。`mergeable=UNKNOWN` / `mergeStateStatus != CLEAN` が原因なら、少し置いて `--execute` を再実行する。再実行時、PR は既に Ready のため `gh pr ready` は**スキップ**され（§12.4）、gate → merge に進む。

### 12.4 `gh pr ready` と「既に Ready」の PR（Nice-to-Have 3）

- `gh pr ready` は **Draft を Ready にする**コマンド。**既に Ready の PR に対しては失敗する**ことがある（"already ready" 系エラー）。
- そのため `approved_merge` は `--execute` 時、冒頭で取得した `isDraft` を見て **Draft のときだけ `gh pr ready` を実行**し、**既に Ready ならスキップ**する（無用な停止を避ける）。スキップ時は「既に Ready のためスキップ」と表示する。
- 運用上も、`--execute` 前に gate（または `gh pr view`）で Draft/Ready 状態を確認しておくとよい。Draft 運用（PR運用ルール v1.0 §4）では通常 Draft で作成 → 承認後にこのツールで Ready 化する流れになる。
...
- **`mergeStateStatus != CLEAN`（空/UNKNOWN 含む）では `READY_CANDIDATE` にならないこと**（Should Fix 1）
- **`docs-only` / `test-only` + base=production が `BLOCKED` になること**（Should Fix 2）
- **実行コードで `--auto` / `--delete-branch` を使っていないこと**（静的 grep）
- **Ready 化後の再 gate が `BLOCKED` のとき merge しないこと**（mock gh で flip 検証）

現状の結果: **PASS=67, FAIL=0**。
...
| 2026-06-08 | v1.1 PR #196 レビュー (A-/Conditional Go) 反映。Should Fix 1: `mergeStateStatus=CLEAN` を gate 判定に追加。Should Fix 2: docs-only/test-only + base=production を NG(BLOCKED) 化。Nice-to-Have: `--auto` 静的 grep テスト / Ready 後再 gate BLOCKED で merge しないテスト / 既に Ready の PR は `gh pr ready` をスキップ（§12.4）。テスト 67 件 PASS。 |
