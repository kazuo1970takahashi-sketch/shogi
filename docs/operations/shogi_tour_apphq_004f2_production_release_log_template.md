...
### 初回 `release_logs/` ディレクトリ作成許可条件（Nice to Have 2 反映）

- 通常、本テンプレート 004F-2 の merge では `release_logs/` ディレクトリ実物は作らない
- **初回の release log 作成タスク**（004F-2 §21 依頼テンプレを使う最初のタスク）では、`docs/operations/release_logs/` ディレクトリを **新規作成してよい**
- ただし以下の条件をすべて満たすこと：
  - release log 作成タスクの **明示承認文に「初回ディレクトリ作成を含む」旨が記載** されている、または承認文の Approval Phrase に対応する
  - **main 側のみ**で作成（production には含めない）
  - production allowlist には追加しない
  - ディレクトリ作成は main 側 **docs-only** として扱う（Level 1〜2）
  - 初回 release log ファイルと同じ commit / 同じ PR で導入してよい
- 2 回目以降の release log 作成では、既存の `release_logs/` ディレクトリ配下にファイル追加するのみ
...
### release 結果別の SHA 記入例（Nice to Have 1 反映）

release result が `Go` 以外の場合、`new production SHA after release` 等の値は変わる。

| release result | previous production SHA | new production SHA after release | rollback 後 production SHA |
|---|---|---|---|
| **Go**（通常成功） | `<release 前の full SHA>` | `<release commit の full SHA>`（previous と異なる） | N/A |
| **Partial**（一部実行・push 成功） | `<release 前の full SHA>` | `<release commit の full SHA>`（previous と異なる、ただし HEAD 確認が一部未完了等） | N/A（rollback 未実施なら） |
| **No Go**（precondition / 検査で停止、commit / push 未実行） | `<release 前の full SHA>` | **N/A**（commit 未作成） | N/A |
| **No Go**（commit 作成済み、push 前停止） | `<release 前の full SHA>` | `<local commit の full SHA>`（push されていないため remote 反映なし、備考に「local commit only, not pushed」と記録） | N/A |
| **Rolled Back**（push 後に rollback、production rollback） | `<release 前の full SHA>` | `<release commit の full SHA>`（一旦は作成された） | `<rollback 後の full SHA>`（previous に戻している場合は previous と同じ） |
| **Rolled Back**（Pages 停止のみ、production tree は維持） | `<release 前の full SHA>` | `<release commit の full SHA>` | N/A（production branch は無変化、Pages 停止のみで対応） |

### 補足

- **No Go** で commit 未作成の場合は `new production SHA after release: N/A` と明示し、§17 Notes に「commit not created, stopped at <stage>」を記録
- **Rolled Back** の場合は **§12 Rollback Policy** で `rollback executed: yes`、`rollback task ID` を必須記入
- rollback 後 production SHA がある場合は `Final State` (§15) に rollback 後 SHA を記録する
- 「N/A」は **空欄ではなく明示的に `N/A` と書く**（取り忘れと判別するため）
...
### GET の扱いを明確化（Should Fix 2 反映）

GitHub API metadata GET と public Pages URL への HTTP GET は **別物** として記録する：

- **GitHub API metadata GET（許可・実施され得る）**：`gh api repos/.../pages` の GET、`gh api .../actions/runs` の GET、`gh pr view --json` 等。これらは構造化された metadata 取得であり、Pages 公開 URL の HTML body 取得とは別。release 実行中・確認中に通常使う。
- **public Pages URL への HTTP GET（未実行）**：`https://kazuo1970takahashi-sketch.github.io/shogi/` 等への `curl` without `-I`、`curl -X GET`、`wget`、browser 表示等は **絶対に未実行**。Pages の動作確認は `curl -I --max-time 20` の HEAD のみ。
- **raw URL / file body 取得（未実行）**：`raw.githubusercontent.com` への GET、file body の保存も未実行。
- 監査ログ上の主記録は **「response body を取得していない / 保存していない」** とする。GET 表現は上記 2 系統を分けて記録する。

### 列挙

- **Pages source 変更していない**（**GitHub API metadata GET のみ実行**、PUT/POST/PATCH/DELETE は未実行）
...
- **public Pages URL への HTTP GET していない**（HEAD のみ、`curl -I --max-time 20`）
  - 注：GitHub API metadata GET（`gh api`）は実施され得るが、これは別系統で許可範囲内
- **`curl -L` / `curl -X GET` / `wget` 不使用**
- **public Pages URL / raw URL / file body 取得していない / 保存していない**
- **browser 表示していない**
- **screenshot 取得していない**
...
### release log 作成時の precondition 確認の意味（Should Fix 1 反映）

**重要：release log 作成タスクは「release 実行が完了した後」のタスク**。release 実行時の precondition（004F-1 §6 の 15 項目）を **再実行するのではない**。再実行すると以下の誤読が発生する：

- release 後の `origin/production` SHA は **release 時の `current production SHA`（= previous production SHA）から変わっている**
- もし 004F-1 §6 の項目 5「`git rev-parse origin/production == CURRENT_PRODUCTION_SHA`」を release log 作成時に再実行すると、現在の SHA が「release前の SHA」と一致しないため **異常と誤読** される
- 実際は release が正常完了して production が進んだ結果であり、異常ではない

#### 正しい precondition 取り扱い

| 項目 | 取り扱い |
|---|---|
| 004F-1 §6 の 15 項目の PASS/FAIL | release **実行時** の完了報告から **転記・照合**（再実行ではない） |
| previous production SHA | release 前の値、転記元は release 完了報告 |
| new production SHA after release | release 後の値、転記元は release 完了報告 |
| release 後の現在状態 | §15 **Final State** セクションで確認する |
| 現在の `origin/production` SHA | **new production SHA after release** と一致しているか確認する |
| 現在の `origin/main` SHA | release は main を変更しないため、approved main SHA と一致しているはず |
| Pages source / status | 現状確認、`production:/, built` 維持を §15 に記録 |

#### 誤読防止ルール

- **release log 作成時に「current production SHA == previous production SHA」を期待しない**（release 後は当然不一致）
- **「previous production SHA」と「post-release の現在の production SHA」を直接照合しない**
- Final State の「現在の production SHA」と「new production SHA after release」を照合する（こちらは一致するはず）
...
入力（release 完了報告から転記）：
- approved main SHA: <APPROVED_MAIN_FULL_SHA>
- previous production SHA: <PREVIOUS_PRODUCTION_FULL_SHA>（release 前の値）
- new production SHA after release: <NEW_PRODUCTION_FULL_SHA>（release 後の値、現在の origin/production と一致するはず）
...
初回 release log 作成の場合：
- docs/operations/release_logs/ ディレクトリは未作成のはず
- 本タスクで新規作成してよい（§2「初回 release_logs/ ディレクトリ作成許可条件」参照）
...
作業手順：
1. precondition 確認（**再実行ではなく転記・照合**）：
   - 004F-1 §6 の 15 項目は release 完了報告から **転記**
   - release **実行時点の** PASS/FAIL を log に記録する
   - 現在の origin/main / origin/production / Pages 状態は §15 Final State に記録（§6 precondition とは別欄）
   - **「現在の origin/production == previous production SHA」を期待しない**（release 後は不一致が正常）
   - 「現在の origin/production == new production SHA after release」が一致を期待する条件
