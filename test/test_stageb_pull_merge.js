...
console.log('=== E: tombstone（deleted=true 反映）＋ MERGE-UNDELETE-001 復元（#549）===');
var mE=mkMaster();
...
// MERGE-UNDELETE-001 (#549): クラウド有効(deleted=false)ならローカル deleted=true を復元する（意図的挙動変更）
...
ok(byId(mE2,'mz').deleted===false&&byId(mE2,'mz').deleted_at===null&&rE2.restored===1&&rE2.updated===0,'E2 cloud deleted=false でローカル deleted=true を復元する（restored=1・updated=0）');
...
// deleted_at が null（クラウドで有効）なら active なローカルを tombstone しない（復元は E5 で検証）
var mE4=mkMaster();
...
ok(byId(mE5,'mz').deleted===false&&byId(mE5,'mz').deleted_at===null&&rE5.restored===1,'E5 deleted_at=null（実クラウド形）でローカル deleted=true を復元する（restored=1）');
...
// MERGE-UNDELETE-001 (#549) 追加検証: 復元の非対称・別カウント
var mE7=mkMaster();
var rE7=env.mergeCloudMembersIntoMaster(mE7,[{member_id:'m1',name:'山田太郎',yomi:'やまだ',deleted_at:null}]);
ok(byId(mE7,'m1').deleted!==true&&rE7.restored===0,'E7 active ローカルは復元カウントに数えない（restored=0）');
...
var rE8=env.mergeCloudMembersIntoMaster(mE8,[{member_id:'mz',name:'削除済',yomi:'',deleted_at:'2026-06-25T00:00:00.000Z'}]);
ok(byId(mE8,'mz').deleted===true&&rE8.restored===0,'E8 cloud も deleted なら復元しない（restored=0）');
var mE9={schema_version:1,updated_at:'2026-06-01T00:00:00.000Z',members:[{id:'mz',name:'旧名',yomi:'きゅう',deleted:true,deleted_at:'2026-05-01T00:00:00.000Z',tournament_ids:[]}]};
var rE9=env.mergeCloudMembersIntoMaster(mE9,[{member_id:'mz',name:'新名',yomi:'しん',deleted_at:null}]);
ok(byId(mE9,'mz').deleted===false&&byId(mE9,'mz').name==='新名'&&rE9.restored===1&&rE9.updated===1,'E9 復元＋氏名更新は restored=1/updated=1（別カウント）');
