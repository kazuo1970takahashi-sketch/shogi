...
//   S2. 「変更」の居場所: flow に居る（absolute でない）・winner-row の margin-top:28px 撤去
//       （#941 で卓バッジを撤去／#942 で専用ヘッダ行を廃し理由ラベルの行へ同居）
...
{
  // TABLE-NO-REMOVE-001 (#941) で卓バッジを撤去、PAIRING-CARD-COMPACT-001 (#942) で
  //   「変更」専用のヘッダ行そのものを廃し、理由ラベルの行へ同居させた。
  //   ★ 生きている命題は「**変更ボタンが flow に居る（absolute ではない）**」こと＝S1 とセットで
  //     44px タップ標的を成立させている条件。どの行に居るかが変わってもこの条件は変わらない。
  const labelRowRe=/\+'<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-top:4px">'\s*\n\s*\+'<div>'\+labelsHtml\+'<\/div>'\s*\n\s*\+'<button style="[^"]*" id="chgbtn_'\+cls\+'_'\+i\+'">変更<\/button>'\s*\n\s*\+'<\/div>'/;
  assert(labelRowRe.test(RAW), 'S2-1 変更ボタンは理由ラベルと同じ flow 行に居る（専用ヘッダ行を持たない）');
  assert(!/background:#1F3864[^"]*">第 '\+\(i\+1\)\+' 卓/.test(RAW), 'S2-1b 卓番号バッジは残っていない');
  // ★ #942: 理由ラベルが空でも行ごと消えてはいけない（消えると「変更」が描かれない）。
  //   旧実装の `(labelsHtml?'<div ...>'+labelsHtml+'</div>':'')` が残っていないことを見る。
  assert(!/\+\(labelsHtml\?'<div style="margin-top:4px">'\+labelsHtml\+'<\/div>':''\)/.test(RAW),
    'S2-1c 理由ラベルの有無で行ごと出し分ける旧実装が残っていない');
