...
if(IDX){
  ...
  const SITE_LABELS=['📋 名簿から受付（過去参加者から選ぶ）','＋ 名簿にない新規の方（手入力）','☁ クラウドから取得','報告書を印刷 / PDF保存','⋯ 編集','☁ クラウドへ送信','📋 参加者を名簿に反映','⏹ 配信を停止','このクラスを部分開始'];
...
// PLAYER-SWAP-002 (#763): 編集シートの独立ボタン「🔁 別の人に差し替える」（名前入力を経由しない到達導線）。
//   アプリに導線がある限り、両マニュアルとアプリ内ヘルプが直接導線を案内していることをピンする。
ok(RAW.indexOf('🔁 別の人に差し替える')>=0,'J5 アプリに編集シートの独立差し替えボタン（🔁 別の人に差し替える）');
BOTH.forEach(([name,doc])=>{
  ok(doc.indexOf('🔁 別の人に差し替える')>=0,'J6 '+name+' に独立差し替えボタン（🔁）の記述');
});
{
  const hm3=RAW.match(/var HELP_TEXTS=\{[\s\S]*?\n\};/);
  ok(!!hm3&&hm3[0].indexOf('🔁 別の人に差し替える')>=0,'J7 アプリ内ヘルプに独立差し替えボタン（🔁）の案内');
}
...
ok(RAW.indexOf('🎪 ゲスト大会のため名簿には反映しません')>=0,'K6 📋 参加者を名簿に反映 の押下時説明（ゲスト大会）');
