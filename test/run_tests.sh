...
# ============================================
# ROUND-CLASS-START-004b class UI bind + reset literal cleanup 単体テスト
# ============================================
echo ""
echo "【ROUND-CLASS-START-004b class UI bind + reset literal cleanup】"
if [ -f "$SCRIPT_DIR/test_round_class_start_004b.js" ]; then
  if node "$SCRIPT_DIR/test_round_class_start_004b.js" "$TARGET" > /tmp/round_class_start_004b_out.log 2>&1; then
    ok "ROUND-CLASS-START-004b テスト 全PASS ($(tail -1 /tmp/round_class_start_004b_out.log))"
  else
    ng "ROUND-CLASS-START-004b テスト 失敗"
    cat /tmp/round_class_start_004b_out.log
  fi
else
  warn "test_round_class_start_004b.js が見つからない"
fi
...
# ============================================
# ROUND-CLASS-START-006 classes-driven registration counts 単体テスト
# ============================================
echo ""
echo "【ROUND-CLASS-START-006 classes-driven registration counts】"
if [ -f "$SCRIPT_DIR/test_round_class_start_006.js" ]; then
  if node "$SCRIPT_DIR/test_round_class_start_006.js" "$TARGET" > /tmp/round_class_start_006_out.log 2>&1; then
    ok "ROUND-CLASS-START-006 テスト 全PASS ($(tail -1 /tmp/round_class_start_006_out.log))"
  else
    ng "ROUND-CLASS-START-006 テスト 失敗"
    cat /tmp/round_class_start_006_out.log
  fi
else
  warn "test_round_class_start_006.js が見つからない"
fi
...
# ============================================
# REPORT-UX-001 tournament report date/time input UX 単体テスト
# ============================================
echo ""
echo "【REPORT-UX-001 tournament report date/time input UX】"
if [ -f "$SCRIPT_DIR/test_report_ux_001.js" ]; then
  if node "$SCRIPT_DIR/test_report_ux_001.js" "$TARGET" > /tmp/report_ux_001_out.log 2>&1; then
    ok "REPORT-UX-001 テスト 全PASS ($(tail -1 /tmp/report_ux_001_out.log))"
  else
    ng "REPORT-UX-001 テスト 失敗"
    cat /tmp/report_ux_001_out.log
  fi
else
  warn "test_report_ux_001.js が見つからない"
fi
...
# ============================================
# REPORT-UX-003A configurable prize field 単体テスト
# ============================================
echo ""
echo "【REPORT-UX-003A configurable prize field】"
if [ -f "$SCRIPT_DIR/test_report_ux_003.js" ]; then
  if node "$SCRIPT_DIR/test_report_ux_003.js" "$TARGET" > /tmp/report_ux_003_out.log 2>&1; then
    ok "REPORT-UX-003A テスト 全PASS ($(tail -1 /tmp/report_ux_003_out.log))"
  else
    ng "REPORT-UX-003A テスト 失敗"
    cat /tmp/report_ux_003_out.log
  fi
else
  warn "test_report_ux_003.js が見つからない"
fi
...
# ============================================
# AUTO001-GATE-TEST-PORT-001 gate スクリプト（pr_gate / approved_merge）の shell テスト
...
# ============================================
echo ""
echo "【AUTO001-GATE-TEST-PORT-001 gate スクリプト（orphan-dev / head-CAS / git ls-remote stub / gh api 非依存）】"
if [ -f "$SCRIPT_DIR/test_pr_gate_scripts.sh" ]; then
  if bash "$SCRIPT_DIR/test_pr_gate_scripts.sh" > /tmp/pr_gate_scripts_out.log 2>&1; then
    ok "AUTO001-GATE-TEST-PORT-001 テスト 全PASS ($(tail -1 /tmp/pr_gate_scripts_out.log))"
  else
    ng "AUTO001-GATE-TEST-PORT-001 テスト 失敗"
