...
echo ""
echo "【HELP-UX-006 (STYLE-GUIDE M2) 対局管理＋クラウド in-app ヘルプ（HELP_TEXTS['tournament']/['cloud'] / open-close / XSS / ？ボタン結線×3）】"
if [ -f "$SCRIPT_DIR/test_help_006.js" ]; then
  if node "$SCRIPT_DIR/test_help_006.js" "$TARGET" > /tmp/help_006_out.log 2>&1; then
    ok "HELP-UX-006 テスト 全PASS ($(tail -1 /tmp/help_006_out.log))"
  else
    ng "HELP-UX-006 テスト 失敗"
    cat /tmp/help_006_out.log
  fi
else
...
echo ""
echo "【AUTH-ERROR-MSG-001 ログイン失敗エラー文言の改善（整形/429分岐/生エラー非表示/reject処理）】"
if [ -f "$SCRIPT_DIR/test_auth_error_msg_001.js" ]; then
  if node "$SCRIPT_DIR/test_auth_error_msg_001.js" "$TARGET" > /tmp/auth_error_msg_out.log 2>&1; then
    ok "AUTH-ERROR-MSG-001 テスト 全PASS ($(tail -1 /tmp/auth_error_msg_out.log))"
  else
    ng "AUTH-ERROR-MSG-001 テスト 失敗"; cat /tmp/auth_error_msg_out.log
  fi
else
  warn "test_auth_error_msg_001.js が見つからない"
fi
...
if [ -f "$SCRIPT_DIR/test_master_rebuild_from_cloud_001.js" ]; then
  if node "$SCRIPT_DIR/test_master_rebuild_from_cloud_001.js" "$TARGET" > /tmp/master_rebuild_from_cloud_001_out.log 2>&1; then
    ok "MASTER-REBUILD-FROM-CLOUD Phase1 テスト 全PASS ($(tail -1 /tmp/master_rebuild_from_cloud_001_out.log))"
  else
    ng "MASTER-REBUILD-FROM-CLOUD Phase1 テスト 失敗"; cat /tmp/master_rebuild_from_cloud_001_out.log
  fi
else
  warn "test_master_rebuild_from_cloud_001.js が見つからない"
fi

echo ""
echo "【MASTER-REBUILD-FROM-CLOUD-001 Phase2 オーケストレーション（rebuildMasterFromCloud）】"
if [ -f "$SCRIPT_DIR/test_master_rebuild_from_cloud_001_phase2.js" ]; then
  if node "$SCRIPT_DIR/test_master_rebuild_from_cloud_001_phase2.js" "$TARGET" > /tmp/master_rebuild_from_cloud_001_phase2_out.log 2>&1; then
    ok "MASTER-REBUILD-FROM-CLOUD Phase2 テスト 全PASS ($(tail -1 /tmp/master_rebuild_from_cloud_001_phase2_out.log))"
  else
    ng "MASTER-REBUILD-FROM-CLOUD Phase2 テスト 失敗"; cat /tmp/master_rebuild_from_cloud_001_phase2_out.log
  fi
else
...
echo ""
echo "【IN-APP-MODAL-001 (#606) スライス — 開始後クラスへの遅延追加確認のアプリ内モーダル化（appConfirm・danger なし・_doAddPlayer 集約）】"
if [ -f "$SCRIPT_DIR/test_in_app_modal_add_player_606.js" ]; then
  if node "$SCRIPT_DIR/test_in_app_modal_add_player_606.js" "$TARGET" > /tmp/in_app_modal_add_player_606_out.log 2>&1; then
    ok "IN-APP-MODAL-ADD-PLAYER-606 テスト 全PASS ($(tail -1 /tmp/in_app_modal_add_player_606_out.log))"
  else
    ng "IN-APP-MODAL-ADD-PLAYER-606 テスト 失敗"; cat /tmp/in_app_modal_add_player_606_out.log
  fi
else
  warn "test_in_app_modal_add_player_606.js が見つからない"
fi


echo ""
echo "【IN-APP-MODAL-001 (#606) スライス — 過去参加者パネルのクラス追加/変更 confirm 2件のアプリ内モーダル化】"
if [ -f "$SCRIPT_DIR/test_in_app_modal_pastadd_606.js" ]; then
  if node "$SCRIPT_DIR/test_in_app_modal_pastadd_606.js" "$TARGET" > /tmp/in_app_modal_pastadd_606_out.log 2>&1; then
    ok "IN-APP-MODAL-PASTADD-606 テスト 全PASS ($(tail -1 /tmp/in_app_modal_pastadd_606_out.log))"
  else
    ng "IN-APP-MODAL-PASTADD-606 テスト 失敗"; cat /tmp/in_app_modal_pastadd_606_out.log
  fi
else
  warn "test_in_app_modal_pastadd_606.js が見つからない"
fi
...
echo ""
echo "【IN-APP-MODAL-001 (#606) スライス — FRP（1局目の逐次追加）confirm 3件のアプリ内モーダル化（再入ガード保持）】"
if [ -f "$SCRIPT_DIR/test_in_app_modal_frp_append_606.js" ]; then
  if node "$SCRIPT_DIR/test_in_app_modal_frp_append_606.js" "$TARGET" > /tmp/in_app_modal_frp_append_606_out.log 2>&1; then
    ok "IN-APP-MODAL-FRP-APPEND-606 テスト 全PASS ($(tail -1 /tmp/in_app_modal_frp_append_606_out.log))"
  else
    ng "IN-APP-MODAL-FRP-APPEND-606 テスト 失敗"; cat /tmp/in_app_modal_frp_append_606_out.log
  fi
else
  warn "test_in_app_modal_frp_append_606.js が見つからない"
fi
...
echo ""
echo "【IN-APP-MODAL-001 (#606) スライス — 運営共通キー張り替え確認（クラウドID）confirm 3件のアプリ内モーダル化】"
if [ -f "$SCRIPT_DIR/test_in_app_modal_ops_rekey_606.js" ]; then
  if node "$SCRIPT_DIR/test_in_app_modal_ops_rekey_606.js" "$TARGET" > /tmp/in_app_modal_ops_rekey_606_out.log 2>&1; then
    ok "IN-APP-MODAL-OPS-REKEY-606 テスト 全PASS ($(tail -1 /tmp/in_app_modal_ops_rekey_606_out.log))"
  else
    ng "IN-APP-MODAL-OPS-REKEY-606 テスト 失敗"; cat /tmp/in_app_modal_ops_rekey_606_out.log
  fi
else
  warn "test_in_app_modal_ops_rekey_606.js が見つからない"
fi
...
echo "【MEMBER-ATTR-SNAPSHOT-001 (#607) Phase2-2 当日 結果一覧/報告書への会員属性表示（§6/§8）】"
if [ -f "$SCRIPT_DIR/test_member_attr_display_dayof_607.js" ]; then
  if node "$SCRIPT_DIR/test_member_attr_display_dayof_607.js" "$TARGET" > /tmp/member_attr_display_dayof_607_out.log 2>&1; then
    ok "MEMBER-ATTR-DISPLAY-DAYOF-607 テスト 全PASS ($(tail -1 /tmp/member_attr_display_dayof_607_out.log))"
  else
    ng "MEMBER-ATTR-DISPLAY-DAYOF-607 テスト 失敗"; cat /tmp/member_attr_display_dayof_607_out.log
  fi
else
...
if [ -f "$SCRIPT_DIR/test_rebuild_semantics_fullmatch_609.js" ]; then
  if node "$SCRIPT_DIR/test_rebuild_semantics_fullmatch_609.js" "$TARGET" > /tmp/rebuild_semantics_fullmatch_609_out.log 2>&1; then
    ok "REBUILD-SEMANTICS-FULLMATCH-609 テスト 全PASS ($(tail -1 /tmp/rebuild_semantics_fullmatch_609_out.log))"
  else
    ng "REBUILD-SEMANTICS-FULLMATCH-609 テスト 失敗（完全一致導線の差分の可能性）"; cat /tmp/rebuild_semantics_fullmatch_609_out.log
  fi
else
