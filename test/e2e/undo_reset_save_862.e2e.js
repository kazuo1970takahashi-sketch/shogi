...
(async () => {
  ...
  // ---- Codex 4巡目 P1: スナップショットの寿命は「リセット後、最初の save() 成功まで」 ----
  //   この規則が成り立つ限り「UNDO_KEY がある＝リセット後まだ何も保存していない」ので、
  //   起動時に出す undo が間に保存した変更を捨てることがない。
  {
    const pg = await browser.newPage();
    pg.on('dialog', d => d.dismiss().catch(() => {}));
    await pg.goto(TARGET, { waitUntil: 'domcontentloaded' });
    const r = await pg.evaluate(() => {
      __setAppModalTestResolver(function () { return true; });
      state.classes = [{ id: 'A', name: 'Aクラス' }];
      state.players = { A: [{ id: 'p1', name: 'リセット前の人', entry_no: 1 }] };
      save();
      resetAll();
      const afterReset = localStorage.getItem('shogi_undo_snapshot') !== null;
      const storedAfterReset = localStorage.getItem('shogi_v4');
      state.players.A = [{ id: 'p2', name: 'リセット後に入れた人', entry_no: 1 }];
      save();                                                     // ここで失効する
      return { afterReset, storedAfterReset, afterSave: localStorage.getItem('shogi_undo_snapshot') !== null };
    });
    await pg.close();
    ok(r.storedAfterReset === null,
      '[R4-A] 前提: resetAll は save() を呼ばない（この規則の土台）  [実測 ' + (r.storedAfterReset === null ? 'null' : '書かれている') + ']');
    ok(r.afterReset === true, '[R4-A] ★対照: リセット直後は失効していない（undo が使える）');
    ok(r.afterSave === false, '[R4-A] ★リセット後に初めて保存できた時点で失効する  [実測 残存=' + r.afterSave + ']');
  }

  // ---- Codex 4巡目 P1: 未保存のまま閉じても、開き直せば拾い直せる ----
  //   バナーは #reg-msg（一時領域）なので、showMsg を伴う別の操作で導線ごと消える。
  //   旧版はそのまま閉じるとメモリ上だけの復元内容を失っていた。
  {
    ...
    await pg.evaluate(() => {
      __setAppModalTestResolver(function () { return true; });
      state.classes = [{ id: 'A', name: 'Aクラス' }];
      state.players = { A: [{ id: 'p1', name: '取り戻したい人', entry_no: 1 }] };
      save();
      resetAll();
      const real = localStorage.setItem.bind(localStorage);
      localStorage.setItem = function (k, v) { if (k === 'shogi_v4') return; return real(k, v); };
      document.getElementById('reset-undo-btn').click();          // undo → 保存に失敗
      localStorage.setItem = real;
      showMsg('別の操作のメッセージ', 'ok');                        // ここで導線が消える
    });
    const before = await pg.evaluate(() => ({
      gone: !(document.getElementById('reset-undo-btn') || document.getElementById('reset-save-retry-btn')),
      undoKey: localStorage.getItem('shogi_undo_snapshot') !== null
    }));
    ...
    const after = await pg.evaluate(() => {
      ...
      return {
        shown: !!(btn && btn.offsetParent !== null),
        height: rect ? Math.round(rect.height) : 0,
        width: rect ? Math.round(rect.width) : 0,
        cls: btn ? btn.className : '',
        inlineStyle: btn ? btn.getAttribute('style') : null,
        label: btn ? (btn.textContent || '').trim() : '',
        msg: (document.getElementById('reg-msg') || {}).textContent || ''
      };
    });
    const recovered = await pg.evaluate(() => {
      const b = document.getElementById('reset-undo-btn');
      if (b) b.click();
      const st = JSON.parse(localStorage.getItem('shogi_v4') || '{}');
      return (((st.players || {}).A) || []).map(function (p) { return p.name; }).join('/');
    });
    await ctx.close();
    ok(before.gone === true, '[R4-B] 前提: 別の操作をすると一時領域の導線は消える');
    ok(before.undoKey === true, '[R4-B] 前提: それでも UNDO_KEY は残っている');
    ok(after.shown === true, '[R4-B] ★開き直すと拾い直せる（バナーが出る）  [実測 ' + after.msg.replace(/\s+/g, ' ').trim().slice(0, 34) + '…]');
    ok(recovered.indexOf('取り戻したい人') >= 0,
      '[R4-B] ★押すと実際に復旧して保存される  [実測 ' + (recovered || '(空)') + ']');
    // ---- Codex 4巡目 P2×2: STYLE-GUIDE §10.3（44px）/ §2.2（class 指定）/ §4.1（用語）----
    ok(after.height >= 44 && after.width >= 44,
      '[R4-C] ★タップ目標が 44×44px 以上（§10.3）  [実測 ' + after.width + '×' + after.height + 'px]');
    ok(after.cls.indexOf('reset-banner-btn') >= 0 && !after.inlineStyle,
      '[R4-C] ★色・枠をインラインで指定せず class で指定（§2.2）  [実測 class=' + after.cls + ' / style=' + after.inlineStyle + ']');
  }

  // ---- Codex 4巡目 P2: localStorage への保存は「自動保存」と呼ぶ（§4.1・「保存」単独は禁止）----
  {
    const pg = await browser.newPage();
    pg.on('dialog', d => d.dismiss().catch(() => {}));
    await pg.goto(TARGET, { waitUntil: 'domcontentloaded' });
    const label = await pg.evaluate(() => {
      ...
      undoLastReset();
      localStorage.setItem = real;
      const b = document.getElementById('reset-save-retry-btn');
      return b ? (b.textContent || '').trim() : '(ボタンなし)';
    });
    await pg.close();
    ok(label.indexOf('自動保存') >= 0,
      '[R4-D] ★再試行ボタンは「自動保存」の語を使う（§4.1）  [実測 ' + label + ']');
    ok(!/(^|[^動])保存する/.test(label),
      '[R4-D] 「保存」単独の言い方をしない（バックアップ保存との取り違え防止）');
  }
