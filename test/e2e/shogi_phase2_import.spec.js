...
test.describe('Phase 2 fix: ヘッダー「大会データを全リセット」リネーム (PR #118)', () => {
  test('ヘッダーボタン textContent が「大会データを全リセット」', async ({ page }) => {
    await setup(page, EMPTY_MASTER);
    const text = await page.locator('#resetBtn').textContent();
    expect(text.trim()).toBe('大会データを全リセット');
  });
  ...
  test('ヘッダーボタン押下で state リセット(マスタ不変、機能変更なし)', async ({ page }) => {
    ...
    // ヘッダーの「大会データを全リセット」ボタン押下 → confirm accept
    page.once('dialog', async (dialog) => { await dialog.accept(); });
