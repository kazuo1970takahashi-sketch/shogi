...
// 抽出件数の sanity check: 16 (save-verify) + 2 (storage-quota) + 3 (master-verify) + 2 (storage-corrupted) = 23
// SAVE-UX-STATE-RESTORE-HANDLING-IMPL-LIGHT (§25): storage-corrupted 1 → 2（PARSE-MASTER-003 + PARSE-LOAD-003）。
// ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 (§5.1): save-verify 15 → 16（startTournamentForClass 追加）。
assertEq(__EXPAND_BLOCKS.length, 23,
  'T-HELPER-shogi-blocks: shogi_v4.html から 23 個の notifySaveWarning block を抽出');
...
// ----------------------------------------------------------------------------
// ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 (§5.1): save-verify:core 4 → 5（startTournamentForClass 追加）。
...
// kind:'save-verify' の件数 = 16（structural、ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 で 15 → 16）
assertEq(__EXP4_SAVE_VERIFY_BLOCKS.length, 16,
  'T-EXP4-kind-count: kind=save-verify の block は 16 件');

// kind=save-verify + severity=warn のペアが厳密に 16 件（structural）
//   SECTION 13 は save-verify 16 件の存在責務を持つ。block 単位の filter で
...
assertEq(
  ...
  16,
  'T-EXP4-save-verify-with-warn-strict: kind=save-verify + severity=warn の block が 16 件');
...
// SAVE-UX-TEST-STRUCTURAL-MATCH 全体合計確認: save-verify 16 + storage-quota 2 + master-verify 3 = 21
//   __EXPAND_BLOCKS.length は SECTION 12.5 で sanity check 済（T-HELPER-shogi-blocks）。
//   ここでは 3 系統合計の整合を最終確認する。
//   ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 (§5.1): save-verify 15 → 16、3 系統合計 20 → 21。
assertEq(
  __EXP4_SAVE_VERIFY_BLOCKS.length + __EXP6_STORAGE_QUOTA_BLOCKS.length + __EXP7_MASTER_VERIFY_BLOCKS.length,
  21,
  'T-EXP7-3systems-total: 3 系統合計 = 21 件 (save-verify 16 + storage-quota 2 + master-verify 3)');

// ----------------------------------------------------------------------------
// T-EXP7-save-verify-unchanged: save-verify 既存件数維持（PHASE1 で 15 → 16）
// ----------------------------------------------------------------------------

// kind:'save-verify' = 16
assertEq(
  (__EXPAND_SRC.match(/kind:'save-verify'/g) || []).length, 16,
  'T-EXP7-save-verify-still-15: kind:save-verify は依然 16 件');
...
// ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 (§5.1): save-verify 15 → 16、3 系統合計 20 → 21。
assertEq(__EXPAND_BLOCKS.filter(function(b){ return b.kind === 'save-verify'; }).length, 16,
  'T-EXP8-save-verify-count-unchanged: save-verify 16 件維持');
...
// 既存 3 系統の合計が 21 件のまま（4 系統合計は SECTION 18 / T-HELPER-shogi-blocks 側で 23 を assert、§25 で +1）
{
  ...
  assertEq(__exp8_sv + __exp8_sq + __exp8_mv, 21,
    'T-EXP8-3systems-total-unchanged: 既存 3 系統 (save-verify+storage-quota+master-verify) 合計 21 件維持');
}
...
// 4 系統 metadata 件数（合計 23、§25 で storage-corrupted 1 → 2、PHASE1 で save-verify 15 → 16）
// ROUND-CLASS-SCOPE-IMPL-LIGHT-PHASE1 (§5.1): save-verify 15 → 16、4 系統合計 22 → 23。
assertEq(__EXPAND_BLOCKS.filter(function(b){ return b.kind === 'save-verify'; }).length, 16,
  'T-EXP9-static-save-verify-15: save-verify 16 件維持');
assertEq(__EXPAND_BLOCKS.filter(function(b){ return b.kind === 'storage-quota'; }).length, 2,
...
assertEq(__EXPAND_BLOCKS.length, 23,
  'T-EXP9-static-4systems-total: 4 系統合計 23 件');
