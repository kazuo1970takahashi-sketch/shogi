...
function gate(src, allow, emit, log) {
  ...
  //   「台帳の名前のうち **いまもトップレベル関数として実在するもの** の本数」を下限にする。
  //     - #798 の掃除: 関数がファイルから消える → 生存数も一緒に減る → 台帳は無改変で緑
  //     - §1 の事故: 関数は実在したまま到達可能へ化ける → 生存数は減らない → 赤
  //     - 通常の PR（生きた関数を足す / 新しい死にコードを allowlist へ足す）: 下限は動かない
  //     - 混在 PR（生きた関数を足しつつ死にコードを 1 本消す）も誤検知しない
  //       （関数総数の増減ではなく**台帳の名前の実在**で数えるため。関数総数の差分を
  //        許容量に使う設計だと、この混在 PR が単独で赤くなる。実測して捨てた）
  ...
  //     塞げていない = **台帳の枯渇**。#798 の掃除が全件終わると生存数が 0 になり、
  //       下限が 0 の恒真 assert になる。そのときスナップショットを取り直す運用が要る
  ...
  log(`  静的到達不能の台帳: ${suNames.length} 件中 いまも実在 ${suAlive.length} 件`
    + `（実測の静的到達不能 ${a.unreachableStatic.length} 件）`);
  emit(a.unreachableStatic.length >= suAlive.length,
    `WI-10 静的到達不能の実測が台帳の生存分を下回らない（実測 ${a.unreachableStatic.length}`
    + ` / 台帳 ${suNames.length} 件中 実在 ${suAlive.length} 件）＝台帳が黙って痩せたことの申告`);
...
console.log('=== WI-10 の検出力（自己検査） ===');
{
  ...
  ok(w10Fails.some((m) => m.startsWith('WI-10 ')),
    `WI-10-SELF-N 台帳の生存分を実測が下回る合成世界で WI-10 が落ちる（実測で落ちた WI-10 系: ${w10Fails.filter((m) => m.startsWith('WI-10')).length} 本）`);

  // ★正極性: **本番の入力（RAW）** に対して WI-10 が実際に評価されていることを測る。
  //   台帳へ「実在するが到達可能な関数」を 1 本足すと生存分が実測 +1 になり、WI-10 だけが
  //   落ちる世界になる（value / static_unreachable も 1 増やして WI-10a / WI-10b は緑のまま）。
...
const SCOPE_LIMITS = new Map([
  ...
  ['WI-10 が守れない形（816A で足した実ファイル依存の申告）', 'WI-10 は '
    ...
    + '「いまもトップレベル関数として実在するもの」を下限に使う。よって次は守れない: '
    + '(a) **関数の改名** … 台帳の名前が byName から消えるので「削除された」と同じ扱いになり、'
    + '改名した関数が到達可能へ化けても鳴らない。'
    + '(b) **台帳の枯渇** … #798 の掃除が全件終わると生存分が 0 になり下限 0 の恒真 assert になる。'
    + 'そのときスナップショットを取り直す運用が要る（毎回 census 行に「台帳 N 件中 実在 M 件」を'
    + '出して枯渇が黙って進まないようにしてある）。'
    + '(c) **台帳より後に足した死にコード** … スナップショットに載っていないので下限に効かない。'
    + '(d) **理由の中身** … WI-10c は字数・根拠参照の形・日付しか見ない（WI-9c と同型の穴）。'
    + 'WI-10-SELF-P は本番の入力（RAW）へ gate() を当て直すので、実ファイルが 0 関数になれば'
    + '空振りする（#816 816A / 2026-08-05）'],
]);
{
