...
// ============================================================
// SECTION L: 最終結果タブ モバイル成績ラベル統一（4 項目すべて全角コロン「：」）
// ============================================================
//   方針: 勝：N / 負：N / 相手：N / 勝相手：N に統一（半角コロン・コロン不在を撤去）
//   実装位置: buildResultsMobileHtml（width=375 でのカード内 inline ラベル）

{
  const m = htmlSrc.match(/function buildResultsMobileHtml\([\s\S]*?\n\}\n/);
  assert(m !== null, 'L0 buildResultsMobileHtml 関数本体を抽出できる');
  const body = m ? m[0] : '';
  // 正：全角コロン付きの 4 項目すべて
  assert(/勝：<strong>/.test(body),
    'L1 「勝：<strong>」 全角コロン付きで存在');
  assert(/負：/.test(body),
    'L2 「負：」 全角コロン付きで存在');
  assert(/相手：/.test(body),
    'L3 「相手：」 全角コロン付きで存在');
  assert(/勝相手：/.test(body),
    'L4 「勝相手：」 全角コロン付きで存在');
  // 旧：半角コロン / コロンなしが残っていないこと
  assert(/相手:[^：]/.test(body) === false,
    'L5 半角コロン「相手:」が残っていない');
  assert(/勝相手:[^：]/.test(body) === false,
    'L6 半角コロン「勝相手:」が残っていない');
  // 旧コロン不在の「勝<strong>」（コロンなし）が残っていないこと
  assert(/勝<strong>/.test(body) === false,
    'L7 コロンなしの「勝<strong>」が残っていない');
  // 旧コロン不在の「　負N」（直後が数字 or 変数で「：」がない）パターンが残っていないこと
  //   既存実装は `'　負'+(f.played-f.A)+'`、新実装は `'　負：'+(f.played-f.A)+'`
  assert(/　負[^：]/.test(body) === false,
    'L8 コロンなしの「　負<数値>」が残っていない');
}
