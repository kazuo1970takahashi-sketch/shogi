...
// ------------------------------------------------------------
// B'. BULK-ENTRY-DELIM-001 (#820): 全角カンマ「，」を区切りとして受理する
//   背景: 日本語入力では「，」になるため、スマホ手打ちが実質できなかった（作者実測 2026-08-06）。
//   区切りは1行につき1種類だけを優先順で選ぶ: タブ ＞ 半角カンマ ＞ 全角カンマ「，」。
//   ★読点「、」は意図的に区切りにしない（氏名の括弧内注記に現実に現れる）。下の B12 がその決定を固定する。
//   ★全角スペースも受理しない（姓名間の空白と衝突する）＝ B7・B13 のとおり従来どおり1列扱い。
// ------------------------------------------------------------
{
  // 受け入れ基準1: 全角カンマ区切りの3列が正しく分かれる（変更を巻き戻すと落ちる）
  const fw = env.parse('鈴木一郎，すずきいちろう，A');
  assert(fw.length === 1 && fw[0].name === '鈴木一郎' && fw[0].yomi === 'すずきいちろう' && fw[0].clsRaw === 'A',
    'B8 全角カンマ「，」区切りの3列を氏名/ふりがな/クラスへ分解（#820・日本語入力の手打ち経路）');

  // 受け入れ基準2: 氏名に読点を含む行は、どの区切りでも分断されない（3形すべて）
  //   案1（[，、] を一律 , へ置換）・案2（読点も区切りに含める）は、いずれかの形でここが落ちる。
  const t1 = env.parse('山田太郎（沼津、東部）');
  assert(t1.length === 1 && t1[0].name === '山田太郎（沼津、東部）' && t1[0].yomi === '' && t1[0].clsRaw === '',
    'B9 区切り無しの1列貼り付けで、氏名の括弧内の読点が分断されない（B4 が明示サポートする経路）');
  const t2 = env.parse('山田太郎（沼津、東部）,やまだたろう,A');
  assert(t2.length === 1 && t2[0].name === '山田太郎（沼津、東部）' && t2[0].yomi === 'やまだたろう' && t2[0].clsRaw === 'A',
    'B10 半角カンマ3列でも氏名の読点が保たれる');
  const t3 = env.parse('山田太郎（沼津、東部），やまだたろう，A');
  assert(t3.length === 1 && t3[0].name === '山田太郎（沼津、東部）' && t3[0].yomi === 'やまだたろう' && t3[0].clsRaw === 'A',
    'B11 全角カンマ3列でも氏名の読点が保たれる（#820 の採用案が案1・案2と分かれる点）');

  // 受け入れ基準3: 読点「、」区切りは分割しない（本案の意図的な非受理）
  const tt = env.parse('佐藤花子、さとうはなこ、B');
  assert(tt.length === 1 && tt[0].name === '佐藤花子、さとうはなこ、B' && tt[0].clsRaw === '',
    'B12 読点「、」は区切りにしない＝1行まるごと氏名（意図的な非受理・氏名の読点を守るため）');

  // 非受理の申し送り: 全角スペースは従来どおり区切りにならない（作者が 8/6 に踏んだ形はこの変更では直らない）
  const sp = env.parse('伊藤三郎　いとうさぶろう　A');
  assert(sp.length === 1 && sp[0].name === '伊藤三郎　いとうさぶろう　A' && sp[0].clsRaw === '',
    'B13 全角スペースは区切りにしない（姓名間の空白と衝突・#820 G1 の申し送り＝この形は本変更では直らない）');

  // タブ優先の維持（Excel 貼り付けの生命線）: セル内に読点・全角カンマがあってもタブが勝つ
  const tab = env.parse('高橋二郎\tたかはし、じろう\tB');
  assert(tab.length === 1 && tab[0].yomi === 'たかはし、じろう' && tab[0].clsRaw === 'B',
    'B14 タブがある行はタブが最優先（セル内の読点はそのまま保たれる）');
}
