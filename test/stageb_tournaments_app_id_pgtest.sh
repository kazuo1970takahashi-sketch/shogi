...
#   stagea_schema + 本 migration + 是正 migration(20260623160000) を使い捨て DB に適用し、
#   列(text,nullable)・非部分一意 index・(club_id, app_tournament_id) の一意enforce
#   （NOT NULL は重複拒否 / NULL は複数可）・ON CONFLICT upsert 成立・冪等を実証。
...
FIXMIG="$MIG_DIR/20260623160000_stageb_tournaments_app_id_fix.sql"
...
if [ ! -f "$FIXMIG" ]; then echo "  ✗ migration が見つからない: $FIXMIG"; echo "  Stage B-2(schema) pgtest: PASS=0 FAIL=1"; exit 1; fi
...
if ! "${PSQL_BASE[@]}" -d "$DB" -f "$FIXMIG" >/tmp/stageb_appid_fix.log 2>&1; then
  echo "  ✗ app_id 是正 migration 適用に失敗:"; cat /tmp/stageb_appid_fix.log; echo "  Stage B-2(schema) pgtest: PASS=0 FAIL=1"; exit 1; fi
ok "schema + app_id + 是正 migration が実 PostgreSQL に適用できる"
...
assert_eq "$(q "select count(*) from pg_indexes where schemaname='public' and indexname='ux_tournaments_club_app_id'")" "1" "一意 index ux_tournaments_club_app_id が存在する"
assert_eq "$(q "select count(*) from pg_index i join pg_class c on c.oid=i.indexrelid where c.relname='ux_tournaments_club_app_id' and i.indpred is null and i.indisunique")" "1" "ux_tournaments_club_app_id は非部分（indpred IS NULL）＝ON CONFLICT のアービターに使える"
...
assert_eq "$(ins_err "$NULLS")" "OK" "app_tournament_id=NULL は複数行を許す（NULLS DISTINCT 既定）"
...
# ON CONFLICT 回帰: PostgREST の upsert と同じ on conflict (club_id, app_tournament_id) が成立する
# （部分一意 index では "no unique or exclusion constraint matching the ON CONFLICT specification" で失敗していた）
ONCONFLICT="insert into public.tournaments(club_id,name,date,season,app_tournament_id) values
  ('$CID','大会甲改','2026-06-14','2026年度','t-onc')
  on conflict (club_id, app_tournament_id) do update set name=excluded.name"
assert_eq "$(ins_err "$ONCONFLICT")" "OK" "ON CONFLICT (club_id, app_tournament_id) upsert が成立する（PostgREST 送信経路の回帰）"

# 冪等: migration 再適用 OK
if "${PSQL_BASE[@]}" -d "$DB" -f "$APPID_MIG" >/tmp/stageb_appid_again.log 2>&1 \
   && "${PSQL_BASE[@]}" -d "$DB" -f "$FIXMIG" >/tmp/stageb_appid_fix_again.log 2>&1; then
  ok "app_id + 是正 migration は冪等（再適用してもエラーにならない）"; else
  ng "app_id/是正 migration 再適用が失敗（冪等でない）"; fi
