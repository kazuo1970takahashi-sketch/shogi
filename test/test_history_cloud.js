...
(async function(){
  ...
  ok(RAW.indexOf("client.from('entries').select('final_rank,class,wins,losses,sos,sodos,member_kind,grade,city,player_id')")>=0,'W3 entries は player_id ベース＋属性列（players 埋め込み回避・#607 で member_kind/grade/city 追加）');
  ok(RAW.indexOf("client.from('players').select('id,member_id,members(name,yomi,member_kind,grade,city)')")>=0,'W3b players を別取得');
