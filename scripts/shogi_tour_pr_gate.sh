...
#
# profile:
#   production-minimal | main-dev | orphan-dev | docs-only | test-only
#   ※ orphan-dev は [AI-DEV-GATES-V1] 追加: orphan clean base 起点の feature PR 用。
#
# 終了コード:
...
#
# [AI-DEV-GATES-V1] 追加点 (re-used auto-001 への最小追記):
#   - profile に orphan-dev を追加（base = orphan clean base branch を要求）。
#   - head SHA (headRefOid) を取得・表示し、承認後 merge の --match-head-commit (CAS) 用に提示する。
#   - 任意 --expect-head <40桁SHA> で「承認時 head == 現在 head」を確認（#235 TOCTOU 早期検出）。
#
...
必須:
  ...
  --profile <profile>    production-minimal | main-dev | orphan-dev | docs-only | test-only
...
任意:
  ...
  --orphan-base-branch <名> [AI-DEV-GATES-V1] orphan-dev profile の base 名
                            (default: chore/shogi-tour-apphq-003h-2d-orphan-clean-base)
  --expect-head <40桁SHA>   [AI-DEV-GATES-V1] 承認時 head の固定確認用 (省略可)
  -h, --help             この使い方を表示
...
valid_profile() {
  case "$1" in
    production-minimal|main-dev|orphan-dev|docs-only|test-only) return 0 ;;
    *) return 1 ;;
  esac
}
...
classify_path() {
  ...
  case "$_cp_profile" in
    ...
    main-dev)
      ...
      ;;
    orphan-dev)
      # [AI-DEV-GATES-V1] orphan clean base 向け feature 開発。
      #   許可範囲は main-dev と同等（data/ のみ禁止）。base 名の違いは base 判定で扱う。
      case "$_cp_path" in
        data/*|*/data/*) echo "FORBIDDEN" ;;
        *) echo "ALLOWED" ;;
      esac
...
fetch_pr_json() {
  ...
  gh pr view "$1" --repo "$2" \
    --json number,state,isDraft,baseRefName,headRefName,headRefOid,mergeable,mergeStateStatus,files
}
...
gate_main() {
  ...
  MAIN_BRANCH="main"
  ORPHAN_BASE_BRANCH="chore/shogi-tour-apphq-003h-2d-orphan-clean-base"  # [AI-DEV-GATES-V1]
  EXPECT_HEAD=""                                                          # [AI-DEV-GATES-V1]
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --orphan-base-branch) [ $# -ge 2 ] || { gate_err "--orphan-base-branch に値が必要です"; return 2; }; ORPHAN_BASE_BRANCH="$2"; shift 2 ;;  # [AI-DEV-GATES-V1]
      --expect-head)      [ $# -ge 2 ] || { gate_err "--expect-head に値が必要です"; return 2; }; EXPECT_HEAD="$2"; shift 2 ;;                  # [AI-DEV-GATES-V1]
  ...
  if ! valid_profile "$PROFILE"; then
    gate_err "不正な profile: '$PROFILE' (production-minimal|main-dev|orphan-dev|docs-only|test-only)"
    return 2
  fi
  case "$PR" in
...
PY
  ...
  echo "  head SHA  : ${PR_HEAD_OID:-UNKNOWN}   # [AI-DEV-GATES-V1] --match-head-commit 用 (CAS)"
  ...
  case "$PROFILE" in
    ...
    main-dev)
      ...
      ;;
    orphan-dev)
      # [AI-DEV-GATES-V1] orphan clean base 起点の feature PR。base 名が orphan base であることを要求。
  ...
  # [AI-DEV-GATES-V1] head SHA guard（任意）: --expect-head 指定時に現在 head との一致を確認。
  #   承認時に固定した head SHA から PR が進んでいないか（#235 TOCTOU）の早期検出。
  #   最終的な原子的 CAS は merge 時の gh pr merge --match-head-commit に委ねる。
  if [ -n "$EXPECT_HEAD" ]; then
    if [ "$EXPECT_HEAD" = "${PR_HEAD_OID:-}" ]; then
      gate_ok "head SHA が --expect-head と一致 ($PR_HEAD_OID)"
    else
      gate_ng "head SHA 不一致: expect=$EXPECT_HEAD / actual=${PR_HEAD_OID:-UNKNOWN}（承認後に head が進んだ可能性 → merge 不可）"
    fi
  fi
...
"
  ...
  case "$_verdict" in
    READY_CANDIDATE)
      ...
      echo ""
      echo "  --- 承認後に実行するコマンド（[AI-DEV-GATES-V1] head SHA を CAS 固定） ---"
      _amerge_extra=""
      if [ "$PROFILE" = "orphan-dev" ]; then _amerge_extra=" --orphan-base-branch $ORPHAN_BASE_BRANCH"; fi
      echo "  dry-run : ./scripts/shogi_tour_approved_merge.sh --pr $PR --profile $PROFILE$_amerge_extra --expect-head ${PR_HEAD_OID:-<headSHA>} --dry-run"
      echo "  実行    : ./scripts/shogi_tour_approved_merge.sh --pr $PR --profile $PROFILE$_amerge_extra --expect-head ${PR_HEAD_OID:-<headSHA>} --execute"
      echo "  ※ merge は gh pr merge --squash --match-head-commit ${PR_HEAD_OID:-<headSHA>} で実行され、"
      echo "     head が進んでいれば GitHub 側で原子的に拒否されます（#235 TOCTOU 防止）。"
      ;;
    NEEDS_REVIEW)
