...
#
# [AI-DEV-GATES-V1] head SHA CAS（re-used auto-001 への最小追記）:
#   - execute 時は gh pr merge を必ず --match-head-commit <現在 head SHA> 付きで実行する。
#     承認後に head が進んでいれば GitHub 側で原子的に merge が拒否される（#235 TOCTOU 防止）。
#   - --expect-head <40桁SHA> を渡すと、merge 直前に「承認時 head == 現在 head」を確認し、
...
任意:
  ...
  --expect-head <40桁SHA>   [AI-DEV-GATES-V1] 承認時 head SHA。merge 直前の一致確認＋--match-head-commit に使用
  -h, --help             この使い方を表示。
...
run_gate() {
  ...
  # [AI-DEV-GATES-V1] orphan-base-branch / expect-head を gate へ委譲（値に空白を含まない前提）。
  _gate_args="--pr $PR --profile $PROFILE --repo $REPO --production-branch $PRODUCTION_BRANCH --main-branch $MAIN_BRANCH --orphan-base-branch $ORPHAN_BASE_BRANCH"
  if [ -n "$EXPECT_HEAD" ]; then _gate_args="$_gate_args --expect-head $EXPECT_HEAD"; fi
  bash "$GATE_SCRIPT" $_gate_args
}
...
amerge_main() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --orphan-base-branch) [ $# -ge 2 ] || { err "--orphan-base-branch に値が必要です"; return 2; }; ORPHAN_BASE_BRANCH="$2"; shift 2 ;;  # [AI-DEV-GATES-V1]
      --expect-head)      [ $# -ge 2 ] || { err "--expect-head に値が必要です"; return 2; }; EXPECT_HEAD="$2"; shift 2 ;;                  # [AI-DEV-GATES-V1]
  ...
  _meta="$(gh pr view "$PR" --repo "$REPO" --json baseRefName,headRefName,headRefOid,state,isDraft 2>/dev/null)"
  ...
  PR_HEAD_OID="$(json_get "$_meta" 'd.get("headRefOid","")')"   # [AI-DEV-GATES-V1]
  ...
  echo "  head SHA     : ${PR_HEAD_OID:-UNKNOWN}   # [AI-DEV-GATES-V1] --match-head-commit (CAS)"
  ...
  if [ "$MODE" = "dry-run" ]; then
    ...
    echo "  3) gh pr merge  $PR --repo $REPO --squash --match-head-commit ${PR_HEAD_OID:-<headSHA>}   # [AI-DEV-GATES-V1] CAS / --delete-branch なし / --auto なし"
  ...
  # --- Step 6: squash merge（[AI-DEV-GATES-V1] --match-head-commit で head SHA を CAS 固定 / --delete-branch なし / --auto なし） ---
  echo ""
  echo "----- Step: gh pr merge --squash --match-head-commit -----"
  # merge 直前に現在 head SHA を再取得（read-only）。--expect-head 指定時は一致を最終確認。
  _merge_head="$(gh pr view "$PR" --repo "$REPO" --json headRefOid -q .headRefOid 2>/dev/null)"
  if [ -z "$_merge_head" ]; then
    err "merge 直前の head SHA を取得できませんでした。安全のため merge せず停止します。"
    return 3
  fi
  if [ -n "$EXPECT_HEAD" ] && [ "$EXPECT_HEAD" != "$_merge_head" ]; then
    err "head SHA が承認時から変化しています: expect=$EXPECT_HEAD / now=$_merge_head"
    err "（承認後に push された可能性。merge せず停止します。再 gate して再承認してください。）"
    return 4
  fi
  note "  head SHA を固定して merge します: --match-head-commit $_merge_head"
  if ! gh pr merge "$PR" --repo "$REPO" --squash --match-head-commit "$_merge_head"; then
    err "gh pr merge に失敗しました（head が直前に変化＝CAS 拒否の可能性。再 gate してください）。"
    return 3
  fi
